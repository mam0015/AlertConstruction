...
const base=1000,builderQuote=base*1.2*1.1,customerDirect=base*1.1;
if(Math.abs(builderQuote-1320)>0.001||Math.abs(customerDirect-1100)>0.001)fail('Builder must apply 20% before 10% GST while Customer Direct applies GST only');

const required=['index.html','.nojekyll','offline.html','AC_Luxury_Builder_Preview.html','login/index.html','login/app.js','legal/privacy.html','legal/terms.html','catalogue/index.html','catalogue/app.js','plan-ai/index.html','quote-analysis/index.html','projects/index.html','renovation-budget/index.html','renovation-budget/rates.js','renovation-budget/app.js','property-estimate/index.html','property-estimate/app.js','property-estimate/victoria-sales-data.js','permit-checklist/index.html','permit-checklist/app.js','shared/auth.js','shared/analytics.js','shared/cloud-sync.js','shared/catalogue-runtime.js','shared/product-shell.js','supabase/migrations/20260714_product_foundation.sql','supabase/migrations/20260714_security_audit_hardening.sql','supabase/migrations/20260715_optional_team_code.sql','supabase/migrations/20260715_privacy_analytics.sql','supabase/functions/analyse-plan/index.ts','assets/app-icon.svg','assets/alert-construction-logo-white.svg','assets/alert-construction-logo-ac.svg','assets/alert-construction-logo-ac-light.svg','assets/ac-monogram-white.png','assets/luxury-builder-hero.webp','assets/luxury-plans.webp','assets/luxury-renovation.webp','assets/luxury-site.webp','assets/luxury-property.webp'];
...
if(!read('login/app.js').includes('Email verified successfully. Your secure account is now active.'))fail('Email verification success state is missing');
for(const marker of ['AUTH_SCRIPT_SRC','authRedirect()','/auth/v1/signup?redirect_to=','/auth/v1/resend?redirect_to=','/auth/v1/recover?redirect_to='])if(!auth.includes(marker))fail(`Deployment-aware email callback is missing ${marker}`);
...
for(const marker of ['ac-action-pricing-v24','brand-logo-light','alert-construction-logo-ac-light.svg','luxury-builder-hero.webp','editorial-grid']){const sources=read('sw.js')+'\n'+home+'\n'+shell+'\n'+read('CHANGES_AND_TESTS.txt');if(!sources.includes(marker))fail(`Luxury builder, action pricing or auth callback mode is missing ${marker}`)}
if((home.match(/class="story"/g)||[]).length!==4)fail('Luxury dashboard must have four selective editorial workflow images');
if(home.includes('class="tool-photo"'))fail('Luxury dashboard tool cards must remain visually clean');
for(const marker of ['luxury-builder-hero.webp','class="stories"','luxury-plans.webp','luxury-renovation.webp','luxury-site.webp','luxury-property.webp'])if(!read('AC_Luxury_Builder_Preview.html').includes(marker))fail(`Luxury Builder HTML preview is missing ${marker}`);
const luxuryPreview=read('AC_Luxury_Builder_Preview.html');
if((luxuryPreview.match(/<article class="tool">/g)||[]).length!==11)fail('Luxury Builder preview must show all 11 dashboard tools');
for(const name of ['Electrical Estimate','Plumbing Estimate','Cladding Estimate','Renovation Budget Planner','Property Value Guide','AI Plan Estimator','Quote Price Analysis','Permit Checklist (Victoria)','Projects & Schedule','Live Price Catalogue','Site Supervisor Checklist'])if(!luxuryPreview.includes(`<h3>${name}</h3>`))fail(`Luxury Builder preview is missing ${name}`);
...
if((home.match(/<a class="tool"/g)||[]).length!==11)fail('Live dashboard must show all 11 existing tools');
...
const victoriaSalesWindow={};Function('window',read('property-estimate/victoria-sales-data.js'))(victoriaSalesWindow);const victoriaSales=victoriaSalesWindow.ACVictoriaSalesData;
if(!victoriaSales||Object.keys(victoriaSales.suburbs||{}).length<700||victoriaSales.suburbs.ROWVILLE?.h?.[0]!==1151500)fail('Official Victorian suburb sales evidence is missing or incomplete');
for(const marker of ['officialEvidence','removeComparableOutliers','No reliable local evidence','ENGINE_VERSION'])if(!read('property-estimate/app.js').includes(marker))fail(`Corrected local property model is missing ${marker}`);
...
const planHtml=read('plan-ai/index.html'),planApp=read('plan-ai/app.js');
for(const marker of ['Existing / Before','Proposed / Future','existingPlanFile','proposedPlanFile','How the comparison works','Who is this price for?','Builder Quote','Customer Direct'])if(!planHtml.includes(marker))fail(`Two-plan estimator UI is missing ${marker}`);
for(const marker of ["mode:'compare'",'existingFileData','proposedFileData','comparison-table','legend-read','action-badge','priceMode','removed_quantity','attachments:[state.plans.existing.file,state.plans.proposed.file]'])if(!planApp.includes(marker))fail(`Two-plan estimator client is missing ${marker}`);
for(const marker of ['PLAN_COMPARE_SCHEMA','FILE A — EXISTING / BEFORE','FILE B — PROPOSED / FUTURE','normalisePlanComparison','added_catalog_index','removed_catalog_index','moved_catalog_index','standard symbol inferred','PLAN_FILE_TYPES'])if(!edge.includes(marker))fail(`Two-plan estimator Edge Function is missing ${marker}`);
for(const trade of ['electrical','plumbing','cladding']){const calculator=read(`${trade}/index.html`);for(const marker of ['Builder Quote (+20%)','Customer Direct (no margin)',"mode==='builder'"])if(!calculator.includes(marker))fail(`${trade} calculator price-mode logic is missing ${marker}`)}
