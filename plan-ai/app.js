(function(){
  ...
  function createResultCard(analysis){
    ...
    const rows=state.items.map((item,index)=>{const product=catalog.items[item.catalog_index],meta=product[2]||{},step=catalog.decimal?'0.01':'1',action=item.action||'added';return `<div class="detected-row" data-item="${index}"><div class="detected-name"><strong><span class="action-badge ${esc(action)}">${esc(action)}</span>${esc(product[0])}<span class="ac-rate-source-badge ${meta.is_custom?'custom':''}">${meta.is_custom?'Your rate':'Default'}</span></strong><small><span class="line-confidence ${esc(item.confidence)}">${esc(item.confidence)} confidence</span>${esc(meta.item_code||'Fixed catalogue item')} • ${esc(item.evidence)}</small></div><div class="quantity"><button type="button" data-step="-${step}">−</button><input type="number" min="0" step="${step}" value="${item.quantity}" aria-label="Quantity"><button type="button" data-step="${step}">+</button></div><div class="line-price">${money(product[1]*item.quantity)}</div></div>`}).join('');
  ...
  window.ACProjectCapture=async function(){
    ...
    const pricedItems=saved.items.map(item=>{const product=CATALOGS[saved.trade].items[item.catalog_index],meta=product[2]||{},unitPrice=Number(product[1]);return{...item,item_code:meta.item_code||'',description:product[0],unit_price_used:unitPrice,default_price_at_time:Number(meta.default_price??unitPrice),price_source:meta.is_custom?'custom':'default',price_list_id:meta.price_list_id||null,price_list_name:meta.price_list_name||null,line_total_ex_gst:unitPrice*item.quantity,captured_at:savedAt}});
    return{module:'plan-estimate',title:name+' — '+CATALOGS[saved.trade].label+' Plan Comparison',summary:saved.symbols.length+' compared symbols • '+saved.items.length+' priced actions • '+money(totals.total)+' '+saved.priceMode+' total',attachments:[state.plans.existing.file,state.plans.proposed.file].filter(Boolean),data:{trade:saved.trade,method:analysis.method||state.method,priceMode:saved.priceMode,planFiles:{existing:state.plans.existing.file?.name||'',proposed:state.plans.proposed.file?.name||''},symbols:saved.symbols,items:pricedItems,analysis:analysis,subtotalExGst:totals.base,profitAmount:totals.profit,gstAmount:totals.gst,finalTotalIncGst:totals.total,builderTotalIncGst:totals.builder,customerTotalIncGst:totals.customer,catalogueAudit:window.ACPriceCatalogue?.security?.()||null,audit:{generatedAt:saved.generatedAt,responseId:saved.responseId,generatedBy:actor,originalItems:saved.originalItems,humanChanges:changes,savedAt,savedBy:actor}}};
  };
})();
