(function(){
  ...
  async function copyCode(){if(window.ACPhotoTimeline&&await window.ACPhotoTimeline.ensureCompanyProfile()===false)return;const code=state.organisation?.join_code;if(!code)return notice('Team Code is unavailable.');try{await navigator.clipboard.writeText(code);notice(`Team Code copied: ${code}`)}catch(_){notice(`Team Code: ${code}`)}}
  async function rotateCode(){if(!confirm('Rotate the Team Code? The old code will stop working immediately.'))return;if(preview){state.organisation.join_code='ATP-'+Math.random().toString(36).slice(2,8).toUpperCase();renderAll();notice(`Sample Team Code rotated to ${state.organisation.join_code}.`);return}await rpc('rotate_ac_join_code');await load();notice('Team Code rotated successfully.')}
