...
function destination(href: string) {
  ...
  if (href.startsWith("/track/")) return `${base}track/?code=${encodeURIComponent(href.slice("/track/".length))}`;
...
export function useRouter() {
  return {
    push(href: string) { window.location.assign(destination(href)); },
...
export function useParams<T extends { code?: string }>() {
  ...
  return { code: queryCode ?? pathCode ?? "" } as T;
}
