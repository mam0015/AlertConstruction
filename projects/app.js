(function(){
  ...
  function renderRecords(project){
    const records=project.records||[];$('recordList').innerHTML=records.length?records.map(record=>{const info=moduleInfo[record.module]||moduleInfo.general,audit=record.data?.audit,changeCount=Array.isArray(audit?.humanChanges)?audit.humanChanges.length:0,auditText=audit?`<span>Audit: ${esc(audit.generatedBy||'authorised user')} • ${changeCount} reviewed change${changeCount===1?'':'s'} • ${new Date(audit.savedAt||record.createdAt).toLocaleString('en-AU')}</span>`:'',attachmentIds=Array.isArray(record.attachmentIds)&&record.attachmentIds.length?record.attachmentIds:record.attachmentId?[record.attachmentId]:[],attachmentNames=Array.isArray(record.attachmentNames)&&record.attachmentNames.length?record.attachmentNames:record.attachmentName?[record.attachmentName]:[],attachmentText=attachmentNames.length?`<span>${attachmentNames.length===1?'Attachment: '+esc(attachmentNames[0]):attachmentNames.length+' attachments: '+attachmentNames.map(esc).join(', ')}</span>`:'';return `<article class="record" data-record="${record.id}"><div class="record-icon">${info[0]}</div><div><strong>${esc(record.title||info[1])}</strong><span>${esc(info[1])} • ${new Date(record.createdAt).toLocaleString('en-AU')}${record.summary?' • '+esc(record.summary):''}</span>${auditText}${attachmentText}</div><div class="record-actions"><button data-action="open">Open</button>${attachmentIds.length?`<button data-action="file">${attachmentIds.length===1?'File':'Files'}</button>`:''}<button class="danger" data-action="delete">Delete</button></div></article>`}).join(''):'<div class="empty-list">No records saved yet. Open an estimator, quote analysis or checklist and choose “Save to Project”.</div>';
  ...
  function openRecord(project,record){
    ...
    if(record.module==='quote-analysis'){localStorage.setItem('ac_project_quote_restore_v1',JSON.stringify({trade:data.trade||'electrical',result:data.result,items:data.items,audit:data.audit}));location.href='../quote-analysis/index.html';return}
