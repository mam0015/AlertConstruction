globalThis.__VINEXT_LAZY_CHUNKS__ = ["assets/BrandLogo-CD9ub4MX.js","assets/OperationsControlPanel-tvTiyK4L.js","assets/TaskManagementPanel-Bx4ap7f4.js","assets/tasks.module-TwnocpNn.js","assets/types-zw-Jr3n5.js","assets/AdminDashboard-RzEk04XW.js","assets/OwnerDashboard-JalyQ6K4.js","assets/page-CXjf6WTE.js","assets/SiteSupervisor-Bj3eDQWH.js","assets/TaskInbox-D515hmDx.js","assets/PendingAccess-DxAF8xH4.js","assets/page-DQ1H0odR.js","assets/WorkerDashboard-BHveGKFq.js","assets/layout-segment-context-UDx8vxfF.js","assets/link-B7lJTf7l.js","assets/router-DaEPvIi4.js"];
...
function getDraftSecret() {
	return "df3337f4-a6a8-444e-9a26-570bc588aecd";
}
/**
...
var NextURL = class NextURL {
	...
	*/
	get buildId() {
		return "2bdfc03c-1d74-4525-acd3-c585fdb08fda";
	}
};
...
*/
function appIsrCacheKey(pathname, suffix, buildId = "2bdfc03c-1d74-4525-acd3-c585fdb08fda") {
...
function createAppPageArtifactCompatibility(element, routePattern) {
	...
	return createArtifactCompatibilityEnvelope({
		...
		deploymentVersion: "2bdfc03c-1d74-4525-acd3-c585fdb08fda",
		rootBoundaryId
	});
}
/**
...
//#endregion
//#region app/page.tsx
var page_exports$7 = /* @__PURE__ */ __exportAll({ default: () => page_default$1 });
...
var metadata = {
	title: "Alert Tradie Pro",
	description: "Request construction and engineering services and track your project online.",
...
//#endregion
//#region db/staff-store.ts
async function database$7() {
	const { env } = await import("cloudflare:workers");
	if (!env.DB) throw new Error("The staff access database is unavailable.");
	return env.DB;
}
async function ensureStaffAccessTable() {
	await (await database$7()).prepare(`CREATE TABLE IF NOT EXISTS staff_access_requests (
...
async function getStaffAccessRequest(email) {
	await ensureStaffAccessTable();
	return (await database$7()).prepare(`SELECT id,email,password_hash AS passwordHash,status,role,trade_title AS tradeTitle,
    requested_at AS requestedAt,reviewed_at AS reviewedAt,last_seen_at AS lastSeenAt
    FROM staff_access_requests WHERE email=? LIMIT 1`).bind(email.toLowerCase()).first();
}
async function listStaffAccessRequests() {
	await ensureStaffAccessTable();
	return (await (await database$7()).prepare(`SELECT id,email,status,role,trade_title AS tradeTitle,
    requested_at AS requestedAt,reviewed_at AS reviewedAt,last_seen_at AS lastSeenAt
    FROM staff_access_requests
    ORDER BY CASE status WHEN 'Pending' THEN 0 WHEN 'Approved' THEN 1 ELSE 2 END, requested_at DESC`).all()).results;
}
async function createStaffAccessRequest(email, passwordHash) {
	await ensureStaffAccessTable();
...
async function touchStaffAccessRequest(email) {
	await ensureStaffAccessTable();
	await (await database$7()).prepare("UPDATE staff_access_requests SET last_seen_at=? WHERE email=?").bind((/* @__PURE__ */ new Date()).toISOString(), email.toLowerCase()).run();
}
async function reviewStaffAccessRequest(id, status, role, tradeTitle) {
	await ensureStaffAccessTable();
	const db = await database$7();
	const reviewedAt = status === "Pending" ? "" : (/* @__PURE__ */ new Date()).toISOString();
	await db.prepare("UPDATE staff_access_requests SET status=?,role=?,trade_title=?,reviewed_at=? WHERE id=?").bind(status, role, tradeTitle.trim(), reviewedAt, id).run();
}
//#endregion
//#region app/secret-utils.ts
var encoder$2 = new TextEncoder();
var PBKDF2_ITERATIONS = 1e5;
function base64UrlEncode$2(input) {
	let binary = "";
	for (const byte of input) binary += String.fromCharCode(byte);
	return btoa(binary).replaceAll("+", "-").replaceAll("/", "_").replaceAll("=", "");
}
function base64UrlDecode$2(input) {
...
function pbkdf2Iterations(storedHash) {
...
async function verifyPbkdf2(value, storedHash) {
	...
	try {
		const salt = base64UrlDecode$2(saltText);
		const expected = base64UrlDecode$2(expectedText);
		if (salt.length < 16 || expected.length !== 32) return false;
		const key = await crypto.subtle.importKey("raw", encoder$2.encode(value), "PBKDF2", false, ["deriveBits"]);
		return safeEqual(new Uint8Array(await crypto.subtle.deriveBits({
			name: "PBKDF2",
			hash: "SHA-256",
			salt,
			iterations: PBKDF2_ITERATIONS
		}, key, expected.length * 8)), expected);
	} catch {
		return false;
	}
}
async function hashPbkdf2(value) {
	...
	return `pbkdf2$${PBKDF2_ITERATIONS}$${base64UrlEncode$2(salt)}$${base64UrlEncode$2(bits)}`;
}
//#endregion
//#region db/access-settings-store.ts
async function database$6() {
	const { env } = await import("cloudflare:workers");
...
function generateTeamCode() {
	...
	return `ATP-${Array.from(bytes, (byte) => alphabet[byte % 32]).join("")}`;
}
async function ensureAccessSettings() {
	const db = await database$6();
	await db.batch([db.prepare(`CREATE TABLE IF NOT EXISTS owner_accounts (
    ...
    (id,email,password_hash,display_name,updated_at) VALUES (1,?,?,?,?)`).bind(ownerEmail, ownerHash, process.env.OWNER_DISPLAY_NAME?.trim() || "Owner", now).run();
	const storedOwner = await db.prepare("SELECT password_hash AS passwordHash FROM owner_accounts WHERE id=1").first();
	if (storedOwner && (pbkdf2Iterations(storedOwner.passwordHash) ?? 0) > 1e5 && isSupportedPbkdf2Hash(ownerHash)) await db.prepare("UPDATE owner_accounts SET password_hash=?,updated_at=? WHERE id=1").bind(ownerHash, now).run();
	const existing = await db.prepare("SELECT id,team_code AS teamCode,team_code_hash AS teamCodeHash FROM team_access_settings WHERE id=1").first();
	if (!existing) {
      ...
      (id,team_code,team_code_hash,updated_at) VALUES (1,?,?,?)`).bind(teamCode, teamCodeHash, now).run();
	} else if (!isSupportedPbkdf2Hash(existing.teamCodeHash)) await db.prepare("UPDATE team_access_settings SET team_code_hash=?,updated_at=? WHERE id=1").bind(await hashPbkdf2(existing.teamCode), now).run();
}
async function getOwnerAccountByEmail(email) {
	await ensureAccessSettings();
	return (await database$6()).prepare(`SELECT email,password_hash AS passwordHash,display_name AS displayName,updated_at AS updatedAt
    FROM owner_accounts WHERE id=1 AND LOWER(email)=LOWER(?) LIMIT 1`).bind(email.trim()).first();
}
async function getPrimaryOwnerAccount() {
	await ensureAccessSettings();
	return (await database$6()).prepare(`SELECT email,password_hash AS passwordHash,display_name AS displayName,updated_at AS updatedAt
    FROM owner_accounts WHERE id=1 LIMIT 1`).first();
}
async function updateOwnerProfile(currentEmail, email, displayName) {
	await ensureAccessSettings();
	const db = await database$6();
	const now = (/* @__PURE__ */ new Date()).toISOString();
	await db.prepare(`UPDATE owner_accounts SET email=?,display_name=?,updated_at=?
    WHERE id=1 AND LOWER(email)=LOWER(?)`).bind(email.trim().toLowerCase(), displayName.trim() || "Owner", now, currentEmail.trim()).run();
	return getPrimaryOwnerAccount();
}
async function updateOwnerPassword(email, password) {
	await ensureAccessSettings();
	await (await database$6()).prepare("UPDATE owner_accounts SET password_hash=?,updated_at=? WHERE id=1 AND LOWER(email)=LOWER(?)").bind(await hashPbkdf2(password), (/* @__PURE__ */ new Date()).toISOString(), email.trim()).run();
}
async function verifyStoredOwnerPassword(email, password) {
	const account = await getOwnerAccountByEmail(email);
	return Boolean(account && await verifyPbkdf2(password, account.passwordHash));
}
async function getTeamAccessSettings() {
	await ensureAccessSettings();
	return (await database$6()).prepare(`SELECT team_code AS teamCode,updated_at AS updatedAt
    FROM team_access_settings WHERE id=1 LIMIT 1`).first();
}
async function verifyCurrentTeamCode(teamCode) {
	await ensureAccessSettings();
	const row = await (await database$6()).prepare("SELECT team_code_hash AS teamCodeHash FROM team_access_settings WHERE id=1").first();
	return Boolean(row && await verifyPbkdf2(teamCode.trim().toUpperCase(), row.teamCodeHash));
}
async function rotateTeamCode() {
	await ensureAccessSettings();
	const db = await database$6();
	const teamCode = generateTeamCode();
	await db.prepare("UPDATE team_access_settings SET team_code=?,team_code_hash=?,updated_at=? WHERE id=1").bind(teamCode, await hashPbkdf2(teamCode), (/* @__PURE__ */ new Date()).toISOString()).run();
	return getTeamAccessSettings();
}
//#endregion
//#region app/admin-auth.ts
var TEAM_COOKIE = "atp_team_session";
var PENDING_COOKIE = "atp_staff_pending";
var SESSION_SECONDS$1 = 3600 * 12;
var PENDING_SECONDS = 3600 * 24 * 7;
...
function base64UrlEncode$1(input) {
	const bytes = typeof input === "string" ? encoder$1.encode(input) : input;
	let binary = "";
	for (const byte of bytes) binary += String.fromCharCode(byte);
	return btoa(binary).replaceAll("+", "-").replaceAll("/", "_").replaceAll("=", "");
}
function base64UrlDecode$1(input) {
	const padded = input.replaceAll("-", "+").replaceAll("_", "/").padEnd(Math.ceil(input.length / 4) * 4, "=");
	const binary = atob(padded);
	return Uint8Array.from(binary, (character) => character.charCodeAt(0));
}
...
var verifyStoredSecret = verifyPbkdf2;
var hashStaffPassword = hashPbkdf2;
var verifyCompanyTeamCode = verifyCurrentTeamCode;
async function hmac$1(value) {
	const key = await crypto.subtle.importKey("raw", encoder$1.encode(requiredTeamSessionSecret()), {
		name: "HMAC",
		hash: "SHA-256"
	}, false, ["sign"]);
	return new Uint8Array(await crypto.subtle.sign("HMAC", key, encoder$1.encode(value)));
}
async function createAdminSession(email, role) {
	const payload = base64UrlEncode$1(JSON.stringify({
		email: email.toLowerCase(),
		role,
		expires: Date.now() + SESSION_SECONDS$1 * 1e3
	}));
	return `${payload}.${base64UrlEncode$1(await hmac$1(payload))}`;
}
async function verifyAdminSession(token) {
	if (!token) return null;
	const [payload, signatureText, extra] = token.split(".");
	if (!payload || !signatureText || extra) return null;
	if (!safeEqual(await hmac$1(payload), base64UrlDecode$1(signatureText))) return null;
	try {
		const parsed = JSON.parse(new TextDecoder().decode(base64UrlDecode$1(payload)));
		if (!parsed.email || !staffRoles.includes(parsed.role) || !parsed.expires || parsed.expires < Date.now()) return null;
...
async function createPendingStaffSession(email) {
	const payload = base64UrlEncode$1(JSON.stringify({
		email: email.toLowerCase(),
		purpose: "staff-approval",
		expires: Date.now() + PENDING_SECONDS * 1e3
	}));
	return `${payload}.${base64UrlEncode$1(await hmac$1(payload))}`;
}
async function verifyPendingStaffSession(token) {
	if (!token) return null;
	const [payload, signatureText, extra] = token.split(".");
	if (!payload || !signatureText || extra) return null;
	if (!safeEqual(await hmac$1(payload), base64UrlDecode$1(signatureText))) return null;
...
function adminSessionCookie(token, secure) {
	return `${TEAM_COOKIE}=${token}; Path=/; HttpOnly; SameSite=Strict; Max-Age=${SESSION_SECONDS$1}${secure ? "; Secure" : ""}`;
}
function clearAdminSessionCookie(secure) {
	return `${TEAM_COOKIE}=; Path=/; HttpOnly; SameSite=Strict; Max-Age=0${secure ? "; Secure" : ""}`;
}
function pendingStaffCookieName() {
	return PENDING_COOKIE;
}
function pendingStaffSessionCookie(token, secure) {
	return `${PENDING_COOKIE}=${token}; Path=/; HttpOnly; SameSite=Strict; Max-Age=${PENDING_SECONDS}${secure ? "; Secure" : ""}`;
}
function clearPendingStaffSessionCookie(secure) {
	return `${PENDING_COOKIE}=; Path=/; HttpOnly; SameSite=Strict; Max-Age=0${secure ? "; Secure" : ""}`;
}
function cookieValue$1(request, name) {
...
async function adminSessionFromRequest(request) {
	return verifyAdminSession(cookieValue$1(request, TEAM_COOKIE));
}
//#endregion
//#region app/owner-auth.ts
var OWNER_COOKIE = "atp_owner_session";
var SESSION_SECONDS = 3600 * 12;
...
async function hmac(value) {
	const key = await crypto.subtle.importKey("raw", encoder.encode(requiredSessionSecret()), {
		name: "HMAC",
		hash: "SHA-256"
	}, false, ["sign"]);
	return new Uint8Array(await crypto.subtle.sign("HMAC", key, encoder.encode(value)));
}
async function isRegisteredOwnerEmail(email) {
	return Boolean(await getOwnerAccountByEmail(email.trim().toLowerCase()));
}
async function ownerDisplayName(email) {
	return (await getOwnerAccountByEmail(email.trim().toLowerCase()))?.displayName ?? "Owner";
}
async function verifyOwnerPassword(email, password) {
...
async function createOwnerSession(email) {
	const payload = base64UrlEncode(JSON.stringify({
		email: email.toLowerCase(),
		expires: Date.now() + SESSION_SECONDS * 1e3
	}));
...
function ownerCookieName() {
	return OWNER_COOKIE;
}
function ownerSessionCookie(token, secure) {
	return `${OWNER_COOKIE}=${token}; Path=/; HttpOnly; SameSite=Strict; Max-Age=${SESSION_SECONDS}${secure ? "; Secure" : ""}`;
}
function clearOwnerSessionCookie(secure) {
	return `${OWNER_COOKIE}=; Path=/; HttpOnly; SameSite=Strict; Max-Age=0${secure ? "; Secure" : ""}`;
}
...
async function ownerSessionFromRequest(request) {
	return verifyOwnerSession(cookieValue(request, OWNER_COOKIE));
}
//#endregion
//#region app/admin/AdminDashboard.tsx
var AdminDashboard_default = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'default' is called on server");
}, "009ec46bb0c1", "default");
//#endregion
...
async function AdminPage({ searchParams }) {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(AdminDashboard_default, {
		viewerName: "Admin",
...
//#endregion
//#region db/owner-store.ts
async function database$5() {
	const { env } = await import("cloudflare:workers");
	if (!env.DB) throw new Error("The owner database is unavailable.");
	return env.DB;
}
async function ensureOwnerDatabase() {
...
async function removeLegacyDemoRecords() {
	...
	if (await db.prepare("SELECT migration_key FROM app_migrations WHERE migration_key=?").bind("remove_seed_records_v72").first()) return;
	await db.batch([
      ...
      WHERE email LIKE '%@alerttradiepro.demo' AND password_hash='preview'`),
		db.prepare("INSERT INTO app_migrations (migration_key,applied_at) VALUES (?,?)").bind("remove_seed_records_v72", (/* @__PURE__ */ new Date()).toISOString())
	]);
}
async function getOwnerSnapshot() {
	await ensureOwnerDatabase();
	await removeLegacyDemoRecords();
	const db = await database$5();
	const r = await db.batch([
		db.prepare(`SELECT id,code,name,service,stage,progress,contract_value AS contractValue,balance,customer_name AS customerName,suburb,start_date AS startDate,notes,updated_at AS updatedAt FROM projects ORDER BY updated_at DESC,id DESC`),
		...
		db.prepare(`SELECT id,person,role,project_code AS projectCode,summary,submitted_at AS submittedAt,status,owner_note AS ownerNote FROM eod_reports ORDER BY submitted_at DESC,id DESC`),
		db.prepare(`SELECT id,sender,recipient,body,sent_at AS sentAt FROM team_messages ORDER BY sent_at,id`),
		db.prepare(`SELECT role,projects,schedule,finance,finance_export AS financeExport,updated_at AS updatedAt FROM role_permissions WHERE role='Admin' LIMIT 1`)
	]);
	return {
		projects: r[0].results,
		financeEntries: r[1].results,
		scheduleEvents: r[2].results,
		eodReports: r[3].results,
		messages: r[4].results,
		permissions: r[5].results[0] ?? {
			role: "Admin",
			projects: 1,
			schedule: 1,
			finance: 0,
		...
		staffRequests: await listStaffAccessRequests()
	};
}
function txt(v, f = "") {
...
async function createOwnerRecord(resource, p) {
	await ensureOwnerDatabase();
	const db = await database$5(), now = (/* @__PURE__ */ new Date()).toISOString();
	if (resource === "project") await db.prepare("INSERT INTO projects (code,name,service,stage,progress,contract_value,balance,customer_name,suburb,start_date,notes,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)").bind(txt(p.code).toUpperCase(), txt(p.name), txt(p.service), txt(p.stage, "Admin review"), Math.min(100, Math.max(0, num(p.progress))), Math.max(0, num(p.contractValue)), Math.max(0, num(p.balance)), txt(p.customerName), txt(p.suburb), txt(p.startDate), txt(p.notes), now).run();
	else if (resource === "finance") await db.prepare("INSERT INTO finance_entries (type,category,project_code,amount,entry_date,note,created_at) VALUES (?,?,?,?,?,?,?)").bind(txt(p.type), txt(p.category), txt(p.projectCode, "Business / General"), Math.max(1, num(p.amount)), txt(p.entryDate, now.slice(0, 10)), txt(p.note), now).run();
	else if (resource === "schedule") await db.prepare("INSERT INTO schedule_events (event_date,start_time,title,assignee,project_code,tone,notes) VALUES (?,?,?,?,?,?,?)").bind(txt(p.eventDate), txt(p.startTime), txt(p.title), txt(p.assignee), txt(p.projectCode, "Business / General"), txt(p.tone, "gold"), txt(p.notes)).run();
	else if (resource === "message") await db.prepare("INSERT INTO team_messages (sender,recipient,body,sent_at) VALUES (?,?,?,?)").bind("Owner", txt(p.recipient, "Site Supervisor 01"), txt(p.body), now).run();
	else throw new Error("This record type cannot be created.");
}
async function updateOwnerRecord(resource, id, p) {
	await ensureOwnerDatabase();
	const db = await database$5(), now = (/* @__PURE__ */ new Date()).toISOString();
	if (resource === "project") await db.prepare("UPDATE projects SET code=?,name=?,service=?,stage=?,progress=?,contract_value=?,balance=?,customer_name=?,suburb=?,start_date=?,notes=?,updated_at=? WHERE id=?").bind(txt(p.code).toUpperCase(), txt(p.name), txt(p.service), txt(p.stage), Math.min(100, Math.max(0, num(p.progress))), Math.max(0, num(p.contractValue)), Math.max(0, num(p.balance)), txt(p.customerName), txt(p.suburb), txt(p.startDate), txt(p.notes), now, num(id)).run();
	else if (resource === "finance") await db.prepare("UPDATE finance_entries SET type=?,category=?,project_code=?,amount=?,entry_date=?,note=? WHERE id=?").bind(txt(p.type), txt(p.category), txt(p.projectCode, "Business / General"), Math.max(1, num(p.amount)), txt(p.entryDate), txt(p.note), num(id)).run();
	else if (resource === "schedule") await db.prepare("UPDATE schedule_events SET event_date=?,start_time=?,title=?,assignee=?,project_code=?,tone=?,notes=? WHERE id=?").bind(txt(p.eventDate), txt(p.startTime), txt(p.title), txt(p.assignee), txt(p.projectCode), txt(p.tone, "gold"), txt(p.notes), num(id)).run();
	else if (resource === "report") await db.prepare("UPDATE eod_reports SET status=?,owner_note=? WHERE id=?").bind(txt(p.status), txt(p.ownerNote), num(id)).run();
	else if (resource === "permission") await db.prepare("UPDATE role_permissions SET projects=?,schedule=?,finance=?,finance_export=?,updated_at=? WHERE role='Admin'").bind(num(p.projects) ? 1 : 0, num(p.schedule) ? 1 : 0, num(p.finance) ? 1 : 0, num(p.financeExport) ? 1 : 0, now).run();
	else if (resource === "staff") {
		const role = txt(p.role, "Unassigned");
		if (role !== "Unassigned" && !staffRoles.includes(role)) throw new Error("Select a valid staff role.");
		const status = txt(p.status, "Pending");
		if (![
			"Pending",
			"Approved",
			"Rejected"
		].includes(status)) throw new Error("Select a valid access status.");
		await reviewStaffAccessRequest(num(id), status, role, txt(p.tradeTitle));
	} else throw new Error("This record type cannot be updated.");
}
async function deleteOwnerRecord(resource, id) {
	await ensureOwnerDatabase();
	const table = resource === "project" ? "projects" : resource === "finance" ? "finance_entries" : resource === "schedule" ? "schedule_events" : null;
	if (!table) throw new Error("This record type cannot be deleted.");
	await (await database$5()).prepare(`DELETE FROM ${table} WHERE id=?`).bind(id).run();
}
async function getLoginAttempt(key) {
	await ensureOwnerDatabase();
	return (await database$5()).prepare("SELECT failed_count AS failedCount,locked_until AS lockedUntil FROM login_attempts WHERE key=?").bind(key).first();
}
async function recordLoginFailure(key, failedCount, lockedUntil) {
	await ensureOwnerDatabase();
	await (await database$5()).prepare("INSERT INTO login_attempts (key,failed_count,locked_until,updated_at) VALUES (?,?,?,?) ON CONFLICT(key) DO UPDATE SET failed_count=excluded.failed_count,locked_until=excluded.locked_until,updated_at=excluded.updated_at").bind(key, failedCount, lockedUntil, Date.now()).run();
}
async function clearLoginFailures(key) {
	await ensureOwnerDatabase();
	await (await database$5()).prepare("DELETE FROM login_attempts WHERE key=?").bind(key).run();
}
//#endregion
//#region db/admin-store.ts
async function database$4() {
	const { env } = await import("cloudflare:workers");
	if (!env.DB) throw new Error("The operations database is unavailable.");
	return env.DB;
}
async function seedAdminRequests() {
	if (process.env.ATP_DEMO_SEED !== "true") return;
	await ensureOwnerDatabase();
	const db = await database$4();
	const existing = await db.prepare("SELECT COUNT(*) AS total FROM job_requests").first();
	...
	await db.batch([
		[
			...
			"Home Extension",
			"Glen Waverley",
			"2026-08-09T08:42:00+10:00",
			"New",
			"High",
			"Ground-floor extension, new family area and internal reconfiguration.",
			"Admin 01"
		],
		[
			"JOB-2026-0221",
			"Job Request",
			"Customer 221",
			"customer221@example.com",
			"Building Inspection",
			"Rowville",
			"2026-08-09T09:18:00+10:00",
			"Contacted",
			"Normal",
			"Pre-purchase building inspection requested for this week.",
			"Admin 01"
		],
		[
			"REQ-2026-0151",
			...
			"Engineering",
			"Wantirna",
			"2026-08-09T10:06:00+10:00",
			"Needs review",
			"Urgent",
			"Structural advice required after movement was noticed near the rear opening.",
			"Site Supervisor 01"
		],
		[
			"JOB-2026-0224",
			"Job Request",
			"Customer 224",
			"customer224@example.com",
			"Bathroom Renovation",
			"Wheelers Hill",
			"2026-08-09T10:34:00+10:00",
			"New",
			"Normal",
			"Bathroom renovation enquiry with photos ready to upload.",
			"Unassigned"
		]
	].map((row) => db.prepare("INSERT INTO job_requests (code,request_type,customer_name,contact,service,suburb,submitted_at,status,priority,summary,assigned_to,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)").bind(...row, now)));
}
async function adminPermissions() {
	return await (await database$4()).prepare("SELECT role,projects,schedule,finance,finance_export AS financeExport FROM role_permissions WHERE role='Admin' LIMIT 1").first() ?? {
		role: "Admin",
		projects: 1,
		schedule: 1,
		finance: 0,
...
function number$1(value, fallback = 0) {
	const parsed = Number(value);
	return Number.isFinite(parsed) ? parsed : fallback;
}
async function requirePermission(resource) {
	const permissions = await adminPermissions();
...
async function createAdminRecord(resource, payload) {
	await getOwnerSnapshot();
	await seedAdminRequests();
	...
	const now = (/* @__PURE__ */ new Date()).toISOString();
	if (resource === "project") {
		await db.prepare("INSERT INTO projects (code,name,service,stage,progress,contract_value,balance,customer_name,suburb,start_date,notes,updated_at) VALUES (?,?,?,?,?,0,0,?,?,?,?,?)").bind(text(payload.code).toUpperCase(), text(payload.name), text(payload.service), text(payload.stage, "Admin review"), Math.min(100, Math.max(0, number$1(payload.progress))), text(payload.customerName), text(payload.suburb), text(payload.startDate), text(payload.notes), now).run();
		return;
	}
	if (resource === "request") {
		await db.prepare("INSERT INTO job_requests (code,request_type,customer_name,contact,service,suburb,submitted_at,status,priority,summary,assigned_to,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)").bind(text(payload.code).toUpperCase(), text(payload.requestType, "Job Request"), text(payload.customerName), text(payload.contact), text(payload.service), text(payload.suburb), text(payload.submittedAt, now), text(payload.status, "New"), text(payload.priority, "Normal"), text(payload.summary), text(payload.assignedTo, "Unassigned"), now).run();
		return;
	}
	if (resource === "schedule") {
		await db.prepare("INSERT INTO schedule_events (event_date,start_time,title,assignee,project_code,tone,notes) VALUES (?,?,?,?,?,?,?)").bind(text(payload.eventDate), text(payload.startTime), text(payload.title), text(payload.assignee, "Site Supervisor 01"), text(payload.projectCode, "Business / General"), text(payload.tone, "gold"), text(payload.notes)).run();
		return;
	}
	if (resource === "message") {
		await db.prepare("INSERT INTO team_messages (sender,recipient,body,sent_at) VALUES (?,?,?,?)").bind("Admin", text(payload.recipient, "Site Supervisor 01"), text(payload.body), now).run();
		return;
	}
}
async function updateAdminRecord(resource, id, payload) {
	await getOwnerSnapshot();
	await seedAdminRequests();
	await requirePermission(resource);
	const db = await database$4();
	const now = (/* @__PURE__ */ new Date()).toISOString();
	if (resource === "project") {
		await db.prepare("UPDATE projects SET code=?,name=?,service=?,stage=?,progress=?,customer_name=?,suburb=?,start_date=?,notes=?,updated_at=? WHERE id=?").bind(text(payload.code).toUpperCase(), text(payload.name), text(payload.service), text(payload.stage), Math.min(100, Math.max(0, number$1(payload.progress))), text(payload.customerName), text(payload.suburb), text(payload.startDate), text(payload.notes), now, id).run();
		return;
	}
	if (resource === "request") {
		await db.prepare("UPDATE job_requests SET status=?,priority=?,assigned_to=?,summary=?,updated_at=? WHERE id=?").bind(text(payload.status), text(payload.priority), text(payload.assignedTo), text(payload.summary), now, id).run();
		return;
	}
	if (resource === "schedule") await db.prepare("UPDATE schedule_events SET event_date=?,start_time=?,title=?,assignee=?,project_code=?,tone=?,notes=? WHERE id=?").bind(text(payload.eventDate), text(payload.startTime), text(payload.title), text(payload.assignee), text(payload.projectCode), text(payload.tone, "gold"), text(payload.notes), id).run();
}
async function deleteAdminRecord(resource, id) {
	await requirePermission(resource);
	if (resource !== "schedule") throw new Error("Admin can only remove schedule items.");
	await (await database$4()).prepare("DELETE FROM schedule_events WHERE id=?").bind(id).run();
}
//#endregion
//#region app/api/admin/data/route.ts
...
var errorResponse = (error) => Response.json({ error: error instanceof Error ? error.message.includes("UNIQUE") ? "That code already exists." : error.message : "The change could not be saved." }, { status: 400 });
var developmentPreview$1 = (request) => false;
var emptyPreview = {
	projects: [],
	requests: [],
	scheduleEvents: [],
...
async function GET$12(request) {
	if (!developmentPreview$1(request) && !await authorised(request)) return Response.json({ error: "Admin sign-in required." }, { status: 401 });
	if (developmentPreview$1(request)) return Response.json({ data: emptyPreview }, { headers: { "Cache-Control": "no-store" } });
...
async function POST$15(request) {
	...
	try {
		...
		if (!valid$1(body.resource) || !body.payload) return Response.json({ error: "Invalid record." }, { status: 400 });
...
async function PATCH$3(request) {
	...
	try {
		const body = await request.json();
		if (!valid$1(body.resource) || !Number.isInteger(body.id) || !body.payload) return Response.json({ error: "Invalid record." }, { status: 400 });
...
async function DELETE$1(request) {
	...
	try {
		const body = await request.json();
		if (!valid$1(body.resource) || !Number.isInteger(body.id)) return Response.json({ error: "Invalid record." }, { status: 400 });
...
var route_exports$16 = /* @__PURE__ */ __exportAll({ POST: () => POST$14 });
...
//#endregion
//#region db/workflow-store.ts
async function database$3() {
	const { env } = await import("cloudflare:workers");
...
function required$3(value, label) {
	const result = clean$3(value);
	if (!result) throw new Error(`${label} is required.`);
	return result;
}
function bounded(value, label, max) {
	const result = required$3(value, label).replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F]/g, "");
...
function normaliseCustomerContact(kind, value) {
...
async function customerContactHash(kind, value) {
	const normalised = normaliseCustomerContact(kind, value);
	const key = await crypto.subtle.importKey("raw", new TextEncoder().encode(requiredContactHashSecret()), {
		name: "HMAC",
		hash: "SHA-256"
	}, false, ["sign"]);
...
async function ensureWorkflowDatabase() {
	const db = await database$3();
	await db.batch([
		db.prepare(`CREATE TABLE IF NOT EXISTS projects (id INTEGER PRIMARY KEY AUTOINCREMENT, code TEXT NOT NULL UNIQUE, name TEXT NOT NULL, service TEXT NOT NULL, stage TEXT NOT NULL, progress INTEGER NOT NULL DEFAULT 0, contract_value INTEGER NOT NULL DEFAULT 0, balance INTEGER NOT NULL DEFAULT 0, customer_name TEXT NOT NULL DEFAULT '', suburb TEXT NOT NULL DEFAULT '', start_date TEXT NOT NULL DEFAULT '', notes TEXT NOT NULL DEFAULT '', updated_at TEXT NOT NULL)`),
		db.prepare(`CREATE TABLE IF NOT EXISTS staff_access_requests (id INTEGER PRIMARY KEY AUTOINCREMENT, email TEXT NOT NULL UNIQUE, password_hash TEXT NOT NULL, status TEXT NOT NULL DEFAULT 'Pending', role TEXT NOT NULL DEFAULT 'Unassigned', trade_title TEXT NOT NULL DEFAULT '', requested_at TEXT NOT NULL, reviewed_at TEXT NOT NULL DEFAULT '', last_seen_at TEXT NOT NULL)`),
    ...
    ORDER BY wc.id DESC LIMIT 100`).all();
	const backfill = [];
	for (const row of missing.results) {
		const caseId = Number(row.id);
		const createdAt = String(row.createdAt || (/* @__PURE__ */ new Date()).toISOString());
...
async function event(db, caseId, role, email, eventType, title, detail = "", audience = "internal") {
	await db.prepare("INSERT INTO workflow_events (case_id,actor_role,actor_email,event_type,title,detail,audience,created_at) VALUES (?,?,?,?,?,?,?,?)").bind(caseId, role, email, eventType, title, detail, audience, (/* @__PURE__ */ new Date()).toISOString()).run();
}
async function removeLegacyWorkflowDemo() {
	...
	if (await db.prepare("SELECT migration_key FROM app_migrations WHERE migration_key=?").bind("remove_workflow_demo_v72").first()) return;
	...
	await db.batch([
		...
		db.prepare("INSERT INTO app_migrations (migration_key,applied_at) VALUES (?,?)").bind("remove_workflow_demo_v72", (/* @__PURE__ */ new Date()).toISOString())
	]);
}
async function findCustomerProjectsByContact(kind, value) {
...
async function getWorkflowSnapshot(role, actorEmail) {
	...
	const reports = result[1].results;
	const files = result[2].results.map((file) => ({
		...file,
		url: `/api/workflow/files?id=${file.id}`
	}));
	const estimates = result[3].results;
	const updates = result[4].results;
	const inspections = result[5].results;
	...
	const shaped = cases.map((item) => {
		...
		return {
			...
			files: caseFiles,
			updates: updates.filter((update) => Number(update.caseId) === caseId).map((update) => ({
				...update,
				files: caseFiles.filter((file) => Number(file.updateId) === Number(update.id))
			}))
		};
	});
	const supervisors = result[7].results.map((row) => ({
		email: String(row.email),
		name: String(row.name)
	}));
	if (!supervisors.length && process.env.ATP_DEMO_SEED === "true") supervisors.push({
		email: "supervisor.preview@example.invalid",
		name: "Site Supervisor Preview"
	});
	return {
		cases: shaped,
		events: result[6].results.filter((item) => allowedIds.has(Number(item.caseId))).map((item) => role === "supervisor" ? {
			...item,
			actorEmail: ""
		} : item),
		supervisors,
...
async function oneCase(caseId) {
	const row = await (await database$3()).prepare("SELECT id,stage,project_code AS projectCode,request_code AS requestCode,service,suburb,customer_name AS customerName,assigned_supervisor_email AS assignedSupervisorEmail,assigned_supervisor_name AS assignedSupervisorName FROM workflow_cases WHERE id=? LIMIT 1").bind(caseId).first();
	if (!row) throw new Error("Project workflow was not found.");
	return row;
}
function allow(role, expected) {
	if (role !== expected) throw new Error(`${expected === "supervisor" ? "Site Supervisor" : expected} access is required for this action.`);
}
function requireStage(current, allowed) {
	if (!allowed.includes(String(current))) throw new Error("This action is not available at the current project stage.");
}
function projectCodeFrom(requestCode) {
	const digits = requestCode.match(/(\d{3,})$/)?.[1] ?? String(Date.now()).slice(-5);
	return `ATP-${(/* @__PURE__ */ new Date()).getFullYear()}-${digits.padStart(5, "0")}`;
}
function privateRequestCode() {
	const bytes = crypto.getRandomValues(new Uint8Array(16));
	const token = Array.from(bytes, (byte) => byte.toString(16).padStart(2, "0")).join("").toUpperCase();
	return `REQ-${(/* @__PURE__ */ new Date()).getFullYear()}-${token}`;
}
async function performWorkflowAction(role, actorEmail, action, caseId, payload) {
	await ensureWorkflowDatabase();
	const db = await database$3();
	...
	if (action === "review_started") {
		allow(role, "admin");
		requireStage(stage, ["request_submitted"]);
		...
		allow(role, "admin");
		requireStage(stage, ["request_submitted", "admin_review"]);
		const note = required$3(payload.note, "Contact note");
		...
		allow(role, "admin");
		requireStage(stage, ["admin_review", "customer_contacted"]);
		const projectCode = projectCodeFrom(String(item.requestCode));
		const folder = `projects/${projectCode}`;
		await db.batch([db.prepare("UPDATE workflow_cases SET stage='site_visit_ready',project_code=?,project_folder=?,updated_at=? WHERE id=?").bind(projectCode, folder, now, caseId), db.prepare("INSERT OR IGNORE INTO projects (code,name,service,stage,progress,contract_value,balance,customer_name,suburb,start_date,notes,updated_at) VALUES (?,?,?,?,0,0,0,?,?,?,'Created from approved customer request.',?)").bind(projectCode, `${item.service} · ${item.suburb}`, item.service, "Site inspection", item.customerName, item.suburb, "", now)]);
		...
		const supervisorEmail = required$3(payload.supervisorEmail, "Site Supervisor").toLowerCase();
		const supervisorName = required$3(payload.supervisorName, "Site Supervisor name");
		const visitAt = required$3(payload.visitAt, "Site visit date and time");
		await db.prepare("UPDATE workflow_cases SET stage='site_visit_scheduled',assigned_supervisor_email=?,assigned_supervisor_name=?,site_visit_at=?,updated_at=? WHERE id=?").bind(supervisorEmail, supervisorName, visitAt, now, caseId).run();
		...
		allow(role, "supervisor");
		requireStage(stage, ["site_visit_scheduled", "visit_changes_requested"]);
		...
		const values = [
			caseId,
			actorEmail,
			required$3(payload.visitDate, "Visit date"),
			required$3(payload.summary, "Visit summary"),
			required$3(payload.findings, "Findings"),
			required$3(payload.recommendations, "Recommendations"),
			clean$3(payload.internalNotes),
			"submitted",
			"",
			now,
			""
		];
		const previous = await db.prepare("SELECT id FROM site_visit_reports WHERE case_id=? ORDER BY id DESC LIMIT 1").bind(caseId).first();
		if (previous?.id) await db.prepare("UPDATE site_visit_reports SET supervisor_email=?,visit_date=?,summary=?,findings=?,recommendations=?,internal_notes=?,status='submitted',admin_note='',submitted_at=?,reviewed_at='' WHERE id=?").bind(actorEmail, values[2], values[3], values[4], values[5], values[6], now, previous.id).run();
		else await db.prepare("INSERT INTO site_visit_reports (case_id,supervisor_email,visit_date,summary,findings,recommendations,internal_notes,status,admin_note,submitted_at,reviewed_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)").bind(...values).run();
		...
		allow(role, "admin");
		requireStage(stage, ["site_visit_submitted"]);
		const decision = clean$3(payload.decision);
		if (!["approved", "changes_requested"].includes(decision)) throw new Error("Choose approve or request changes.");
		const note = clean$3(payload.note);
		...
		await db.batch([db.prepare("UPDATE site_visit_reports SET status=?,admin_note=?,reviewed_at=? WHERE case_id=?").bind(decision, note, now, caseId), db.prepare("UPDATE workflow_cases SET stage=?,updated_at=? WHERE id=?").bind(nextStage, now, caseId)]);
		...
		const scope = required$3(payload.scope, "Estimate scope");
		const terms = clean$3(payload.terms);
		await db.batch([db.prepare("INSERT INTO workflow_estimates (case_id,amount_cents,scope,terms,status,created_by,created_at,sent_at,customer_decided_at,confirmed_at) VALUES (?,?,?,?, 'draft', ?, ?, '', '', '') ON CONFLICT(case_id) DO UPDATE SET amount_cents=excluded.amount_cents,scope=excluded.scope,terms=excluded.terms,status='draft',created_by=excluded.created_by,created_at=excluded.created_at,sent_at='',customer_decided_at='',confirmed_at='' ").bind(caseId, amountCents, scope, terms, actorEmail, now), db.prepare("UPDATE workflow_cases SET stage='estimate_ready',updated_at=? WHERE id=?").bind(now, caseId)]);
		...
		allow(role, "admin");
		requireStage(stage, ["estimate_ready"]);
		await db.batch([db.prepare("UPDATE workflow_estimates SET status='sent',sent_at=? WHERE case_id=?").bind(now, caseId), db.prepare("UPDATE workflow_cases SET stage='estimate_sent',updated_at=? WHERE id=?").bind(now, caseId)]);
		...
		allow(role, "admin");
		requireStage(stage, ["customer_approved"]);
		if ((await db.prepare("SELECT status FROM workflow_estimates WHERE case_id=?").bind(caseId).first())?.status !== "customer_accepted") throw new Error("The customer must accept the estimate first.");
		await db.batch([
		...
		const created = await db.prepare("INSERT INTO project_updates (case_id,work_date,supervisor_email,internal_update,customer_update,status,created_at) VALUES (?,?,?,?,?,'pending_admin',?)").bind(caseId, required$3(payload.workDate, "Work date"), actorEmail, required$3(payload.internalUpdate, "Internal team update"), required$3(payload.customerUpdate, "Customer update"), now).run();
		...
		const inspectedAt = required$3(payload.inspectedAt, "Inspection date");
		const summary = required$3(payload.summary, "Quality inspection summary");
		const defects = clean$3(payload.defects);
		await db.batch([db.prepare("INSERT INTO quality_inspections (case_id,supervisor_email,inspected_at,summary,defects,status,submitted_at) VALUES (?,?,?,?,?,'submitted',?) ON CONFLICT(case_id) DO UPDATE SET supervisor_email=excluded.supervisor_email,inspected_at=excluded.inspected_at,summary=excluded.summary,defects=excluded.defects,status='submitted',admin_note='',owner_note='',submitted_at=excluded.submitted_at,reviewed_at='',completed_at='' ").bind(caseId, actorEmail, inspectedAt, summary, defects, now), db.prepare("UPDATE workflow_cases SET stage='quality_inspection',updated_at=? WHERE id=?").bind(now, caseId)]);
		...
		allow(role, "admin");
		requireStage(stage, ["quality_inspection"]);
		const decision = clean$3(payload.decision);
		...
		await db.batch([db.prepare("UPDATE quality_inspections SET status=?,admin_note=?,reviewed_at=? WHERE case_id=?").bind(decision, note, now, caseId), db.prepare("UPDATE workflow_cases SET stage=?,updated_at=? WHERE id=?").bind(nextStage, now, caseId)]);
		...
		allow(role, "owner");
		requireStage(stage, ["completion_ready"]);
		const note = clean$3(payload.note);
		await db.batch([
		...
		if ((await db.prepare("SELECT status FROM project_updates WHERE id=? AND case_id=?").bind(updateId, caseId).first())?.status !== "pending_admin") throw new Error("This update is not waiting for Admin approval.");
		await db.prepare("UPDATE project_updates SET status='pending_owner',admin_note=?,admin_reviewed_at=? WHERE id=?").bind(clean$3(payload.note), now, updateId).run();
		...
		allow(role, "owner");
		const updateId = number(payload.updateId);
		if ((await db.prepare("SELECT status FROM project_updates WHERE id=? AND case_id=?").bind(updateId, caseId).first())?.status !== "pending_owner") throw new Error("Admin approval is required before Owner approval.");
		await db.batch([db.prepare("UPDATE project_updates SET status='published',owner_note=?,owner_reviewed_at=?,published_at=? WHERE id=?").bind(clean$3(payload.note), now, now, updateId), db.prepare("UPDATE workflow_files SET visibility='published',published_at=? WHERE case_id=? AND update_id=?").bind(now, caseId, updateId)]);
		...
		const note = required$3(payload.note, "Closing reason");
		...
		const updateId = number(payload.updateId);
		const note = required$3(payload.note, "Return note");
		await db.prepare("UPDATE project_updates SET status='changes_requested',admin_note=?,owner_note=?,admin_reviewed_at=?,owner_reviewed_at=? WHERE id=? AND case_id=?").bind(role === "admin" ? note : "", role === "owner" ? note : "", role === "admin" ? now : "", role === "owner" ? now : "", updateId, caseId).run();
...
async function createPublicWorkflowRequest(payload) {
	await ensureWorkflowDatabase();
	const db = await database$3();
	const now = (/* @__PURE__ */ new Date()).toISOString();
	const accepted = clean$3(payload.termsAccepted);
	if (![
		"on",
		"true",
		"accepted"
	].includes(accepted.toLowerCase())) throw new Error("Please accept the Privacy Notice and Request Terms before submitting.");
	...
	const otherService = clean$3(payload.otherService).slice(0, 1e3);
	const service = requestedService === "Other" && otherService ? `Other — ${otherService}`.slice(0, 120) : requestedService;
	const material = clean$3(payload.material).slice(0, 120);
	const details = bounded(payload.details, "Project details", 6e3);
	const description = material ? `${details}\n\nMaterial preference: ${material}`.slice(0, 6e3) : details;
	const phone = bounded(payload.phone, "Phone", 40);
	const created = await db.prepare("INSERT INTO workflow_cases (request_code,customer_name,customer_email,customer_phone,service,suburb,description,timeframe,budget,stage,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,'request_submitted',?,?)").bind(code, bounded(payload.name, "Full name", 120), email, phone, service, bounded(payload.location, "Project location", 240), description, clean$3(payload.timeframe).slice(0, 120), clean$3(payload.budget).slice(0, 120), now, now).run();
	...
	await event(db, caseId, "Customer", clean$3(payload.email), "request_submitted", "Customer submitted a new project request", `${clean$3(payload.service)} · ${clean$3(payload.location)}`);
	return {
		caseId,
		code
	};
}
async function getCustomerUploadTarget(caseId, code) {
	await ensureWorkflowDatabase();
	const item = await (await database$3()).prepare(`SELECT id,request_code AS requestCode,customer_email AS customerEmail,
    ...
    FROM workflow_cases WHERE id=? AND UPPER(request_code)=UPPER(?) LIMIT 1`).bind(caseId, code.trim()).first();
	if (!item) throw new Error("The customer request was not found.");
	if (![
		"request_submitted",
		"admin_review",
		"customer_contacted"
	].includes(String(item.stage))) throw new Error("Customer documents can no longer be added to this request.");
...
async function addCustomerRequestFile(caseId, objectKey, file, customerEmail) {
	await ensureWorkflowDatabase();
	const db = await database$3();
	const created = await db.prepare("INSERT INTO workflow_files (case_id,category,object_key,file_name,mime_type,size_bytes,uploaded_by,visibility,uploaded_at) VALUES (?,'customer_request',?,?,?,?,?,'internal',?)").bind(caseId, objectKey, file.name, file.type, file.size, customerEmail, (/* @__PURE__ */ new Date()).toISOString()).run();
...
async function getPublicWorkflow(code) {
	...
	return {
		...
		updates: result[1].results.map((update) => ({
			...update,
			files: files.filter((file) => Number(file.updateId) === Number(update.id))
		})),
		activity: result[3].results
	};
}
async function customerEstimateDecision(code, decision) {
	await ensureWorkflowDatabase();
	const db = await database$3();
	const item = await db.prepare("SELECT id,stage,customer_email AS customerEmail FROM workflow_cases WHERE UPPER(request_code)=? LIMIT 1").bind(code.toUpperCase()).first();
	if (!item) throw new Error("Project was not found.");
	requireStage(item.stage, ["estimate_sent"]);
	const caseId = Number(item.id);
	const now = (/* @__PURE__ */ new Date()).toISOString();
	const accepted = decision === "accept";
	await db.batch([db.prepare("UPDATE workflow_estimates SET status=?,customer_decided_at=? WHERE case_id=? AND status='sent'").bind(accepted ? "customer_accepted" : "customer_declined", now, caseId), db.prepare("UPDATE workflow_cases SET stage=?,updated_at=? WHERE id=?").bind(accepted ? "customer_approved" : "estimate_declined", now, caseId)]);
...
async function addWorkflowFile(caseId, category, objectKey, file, uploadedBy) {
	await ensureWorkflowDatabase();
	const db = await database$3();
	const created = await db.prepare("INSERT INTO workflow_files (case_id,category,object_key,file_name,mime_type,size_bytes,uploaded_by,visibility,uploaded_at) VALUES (?,?,?,?,?,?,?,'internal',?)").bind(caseId, category, objectKey, file.name, file.type, file.size, uploadedBy, (/* @__PURE__ */ new Date()).toISOString()).run();
...
async function getWorkflowFile(id, publicOnly) {
	await ensureWorkflowDatabase();
	const db = await database$3();
	const where = publicOnly ? "id=? AND visibility='published'" : "id=?";
	return db.prepare(`SELECT object_key AS objectKey,mime_type AS mimeType,file_name AS fileName FROM workflow_files WHERE ${where} LIMIT 1`).bind(id).first();
}
//#endregion
//#region db/operations-store.ts
var followUpStatuses = new Set([
	"open",
	"completed",
	"cancelled"
]);
...
function melbourneDate$1(offsetDays = 0) {
...
async function ensureOperationsDatabase() {
	await ensureOwnerDatabase();
	await ensureWorkflowDatabase();
...
function mapIssue(row) {
	return {
		...
		siteLocation: String(row.siteLocation || ""),
		affectedTrade: String(row.affectedTrade || "Other"),
		issueType: String(row.issueType || "Delay"),
		severity: String(row.severity || "High"),
		summary: String(row.summary || ""),
		details: String(row.details || ""),
		impact: String(row.impact || ""),
		contactedPerson: String(row.contactedPerson || ""),
		contactedAt: String(row.contactedAt || ""),
		expectedDate: String(row.expectedDate || ""),
		reporterEmail: String(row.reporterEmail || ""),
		reporterName: String(row.reporterName || "Site Supervisor"),
		status: String(row.status || "reported"),
		adminAction: String(row.adminAction || ""),
		rescheduledDate: String(row.rescheduledDate || ""),
		rescheduledTime: String(row.rescheduledTime || ""),
		rescheduledAssignee: String(row.rescheduledAssignee || ""),
		adminEmail: String(row.adminEmail || ""),
		adminReviewedAt: String(row.adminReviewedAt || ""),
		ownerNote: String(row.ownerNote || ""),
...
function mapFollowUp(row) {
	return {
		...
		personRole: String(row.personRole || ""),
		...
		clockedOutAt: String(row.clockedOutAt || ""),
		createdAt: String(row.createdAt || ""),
		updatedAt: String(row.updatedAt || ""),
		completedAt: String(row.completedAt || "")
	};
}
async function projectRows(db, identity) {
	...
	return (identity.role === "Site Supervisor" ? await db.prepare(`${base} WHERE LOWER(assigned_supervisor_email)=? AND stage IN ('site_visit_scheduled','site_visit_submitted','site_visit_approved','estimate_ready','estimate_sent','customer_approved','active_project') ORDER BY updated_at DESC`).bind(identity.email.toLowerCase()).all() : await db.prepare(`${base} WHERE project_code!='' ORDER BY updated_at DESC`).all()).results.map(mapProject);
}
async function issueRows(db, identity) {
...
async function performOperationsAction(identity, action, payload) {
	await ensureOperationsDatabase();
	const db = await database$2();
	const now = (/* @__PURE__ */ new Date()).toISOString();
	if (action === "create_issue") {
		...
		const project = await db.prepare("SELECT id,project_code AS projectCode,service,suburb FROM workflow_cases WHERE id=? AND LOWER(assigned_supervisor_email)=? LIMIT 1").bind(caseId, identity.email.toLowerCase()).first();
		...
		const affectedTrade = required$2(payload.affectedTrade, "Affected trade");
		const severity = clean$2(payload.severity) || "High";
		if (!severities.has(severity)) throw new Error("Choose a valid severity.");
		const summary = required$2(payload.summary, "Problem summary");
		const siteLocation = required$2(payload.siteLocation, "Project location");
		await db.prepare("INSERT INTO site_issue_reports (case_id,project_code,project_name,site_location,affected_trade,issue_type,severity,summary,details,impact,contacted_person,contacted_at,expected_date,reporter_email,reporter_name,status,reported_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'reported',?,?)").bind(caseId, projectCode, `${String(project.service || "Project")} · ${String(project.suburb || "Site")}`, siteLocation, affectedTrade, required$2(payload.issueType, "Issue type"), severity, summary, clean$2(payload.details), clean$2(payload.impact), clean$2(payload.contactedPerson), clean$2(payload.contactedAt), clean$2(payload.expectedDate), identity.email, identity.name, now, now).run();
		await addWorkflowEvent(db, caseId, identity.role, identity.email, "site_issue_reported", `${affectedTrade} ${clean$2(payload.issueType).toLowerCase() || "issue"}`, `${siteLocation}: ${summary}. Severity: ${severity}.`);
		return;
	}
	if ([
		"review_issue",
		...
		const issue = await db.prepare("SELECT id,case_id AS caseId,project_code AS projectCode,affected_trade AS affectedTrade,reporter_email AS reporterEmail,reporter_name AS reporterName,summary FROM site_issue_reports WHERE id=? LIMIT 1").bind(issueId).first();
		...
		if (action === "owner_note") {
			...
			await db.prepare("UPDATE site_issue_reports SET owner_note=?,updated_at=? WHERE id=?").bind(required$2(payload.ownerNote, "Owner direction"), now, issueId).run();
			await addWorkflowEvent(db, Number(issue.caseId), identity.role, identity.email, "site_issue_owner_direction", `Owner direction: ${String(issue.affectedTrade)}`, clean$2(payload.ownerNote));
			return;
		}
		if (action === "review_issue") {
			await db.prepare("UPDATE site_issue_reports SET status='under_review',admin_action=?,admin_email=?,admin_reviewed_at=?,updated_at=? WHERE id=?").bind(required$2(payload.adminAction, "Review action"), identity.email, now, now, issueId).run();
			await addWorkflowEvent(db, Number(issue.caseId), identity.role, identity.email, "site_issue_reviewed", `Reviewing ${String(issue.affectedTrade)} issue`, clean$2(payload.adminAction));
			return;
		}
		if (action === "reschedule_issue") {
			const rescheduledDate = required$2(payload.rescheduledDate, "New date");
			const rescheduledTime = required$2(payload.rescheduledTime, "New time");
			const assignee = required$2(payload.rescheduledAssignee, "Site Supervisor assignee");
			const actionNote = required$2(payload.adminAction, "Reschedule direction");
			await db.batch([
				...
				db.prepare("INSERT INTO follow_up_items (person_email,person_role,person_name,project_code,title,details,target_date,source,dedupe_key,status,created_by_email,created_by_role,work_date,clocked_out_at,created_at,updated_at,completed_at) VALUES (?,'Site Supervisor',?,?,?,?,?,'site_issue',NULL,'open',?,?,?,'',?,?,'')").bind(String(issue.reporterEmail), String(issue.reporterName), String(issue.projectCode || "Business / General"), `Follow up ${String(issue.affectedTrade)} delay`, actionNote, rescheduledDate, identity.email, identity.role, melbourneDate$1(), now, now)
			]);
		...
		await db.prepare("UPDATE site_issue_reports SET status='resolved',admin_action=?,admin_email=?,admin_reviewed_at=?,resolved_at=?,updated_at=? WHERE id=?").bind(required$2(payload.adminAction, "Resolution"), identity.email, now, now, now, issueId).run();
		await addWorkflowEvent(db, Number(issue.caseId), identity.role, identity.email, "site_issue_resolved", `${String(issue.affectedTrade)} issue resolved`, clean$2(payload.adminAction));
		return;
	}
	if (action === "create_follow_up" || action === "clock_out_follow_up") {
		const targetDate = required$2(payload.targetDate, "Follow-up date");
		const title = required$2(payload.title, "Follow-up task");
		const source = action === "clock_out_follow_up" ? "clock_out" : "manual";
		const workDate = melbourneDate$1();
	...
	if (action === "set_follow_up_status") {
		...
		const bindings = identity.role === "Site Supervisor" ? [
			status,
			now,
		...
		if (!(await db.prepare(`UPDATE follow_up_items SET status=?,updated_at=?,completed_at=? WHERE id=?${ownership}`).bind(...bindings).run()).meta.changes) throw new Error("The follow-up was not found or is not assigned to you.");
		return;
	}
	throw new Error("Unknown operations action.");
}
//#endregion
//#region app/api/operations/route.ts
var route_exports$15 = /* @__PURE__ */ __exportAll({
	GET: () => GET$11,
	POST: () => POST$13
});
async function identity$1(request) {
	...
	if (staff?.role === "Site Supervisor") return {
		email: staff.email,
...
async function GET$11(request) {
	const viewer = await identity$1(request);
...
//#endregion
//#region app/api/owner/data/route.ts
...
var auth = async (r) => Boolean(await ownerSessionFromRequest(r));
var err = (e) => Response.json({ error: e instanceof Error ? e.message.includes("UNIQUE") ? "That project code already exists." : e.message : "The change could not be saved." }, { status: 400 });
var developmentPreview = (r) => false;
async function GET$10(r) {
	if (developmentPreview(r)) return Response.json({ data: {
		projects: [],
		financeEntries: [],
		scheduleEvents: [],
		eodReports: [],
	...
	if (!await auth(r)) return Response.json({ error: "Owner sign-in required." }, { status: 401 });
	return Response.json({ data: await getOwnerSnapshot() }, { headers: { "Cache-Control": "no-store" } });
}
async function POST$12(r) {
	if (!requestIsSameOrigin(r)) return Response.json({ error: "Request blocked." }, { status: 403 });
	if (!await auth(r)) return Response.json({ error: "Owner sign-in required." }, { status: 401 });
	try {
		const b = await r.json();
		if (!valid(b.resource) || !b.payload) return Response.json({ error: "Invalid record." }, { status: 400 });
...
async function PATCH$2(r) {
	if (!requestIsSameOrigin(r)) return Response.json({ error: "Request blocked." }, { status: 403 });
	if (!await auth(r)) return Response.json({ error: "Owner sign-in required." }, { status: 401 });
	try {
		const b = await r.json();
		if (!valid(b.resource) || b.id === void 0 || !b.payload) return Response.json({ error: "Invalid record." }, { status: 400 });
...
async function DELETE(r) {
	if (!requestIsSameOrigin(r)) return Response.json({ error: "Request blocked." }, { status: 403 });
	if (!await auth(r)) return Response.json({ error: "Owner sign-in required." }, { status: 401 });
	try {
		const b = await r.json();
		if (!valid(b.resource) || !Number.isInteger(b.id)) return Response.json({ error: "Invalid record." }, { status: 400 });
...
//#endregion
//#region app/api/owner/logout/route.ts
var route_exports$13 = /* @__PURE__ */ __exportAll({ POST: () => POST$11 });
async function POST$11(request) {
	if (!requestIsSameOrigin(request)) return Response.json({ error: "Request blocked." }, { status: 403 });
	const secure = new URL(request.url).protocol === "https:";
	return Response.json({ ok: true }, { headers: {
		"Set-Cookie": clearOwnerSessionCookie(secure),
		"Cache-Control": "no-store"
	} });
}
//#endregion
//#region app/api/owner/settings/route.ts
	...
	try {
		...
		if (body.action === "team-code") {
			...
			return Response.json({
				data: await settingsPayload(),
				message: "A new Team Code is active."
			});
		}
		if (!body.currentPassword || !await verifyStoredOwnerPassword(session.email, body.currentPassword)) return Response.json({ error: "Current password is incorrect." }, { status: 401 });
		...
		if (body.action === "profile") {
			...
			return Response.json({
				...
				"Set-Cookie": ownerSessionCookie(token, secure),
				"Cache-Control": "no-store"
			} });
		}
...
//#endregion
//#region app/team-access-notification.ts
function setting(name) {
	return process.env[name]?.trim() ?? "";
}
async function notifyOwnerOfStaffRequest(staffEmail) {
	...
	})).ok) throw new Error("Owner notification provider rejected the request.");
}
//#endregion
//#region app/staff-access.ts
function staffRedirect(role) {
	if (role === "Admin") return "/admin";
	if (role === "Site Supervisor") return "/site-supervisor";
	if (isWorkerRole(role)) return "/worker";
	return "/team/workspace";
}
var workerStaffRoles = [
	"Worker",
	"Electrician",
	"Plumber",
	"Cleaner",
...
function isWorkerRole(role) {
	return workerStaffRoles.includes(role);
}
async function processStaffSignIn(emailInput, password, teamCode) {
	...
	if (!existing) {
		...
		await notifyOwnerOfStaffRequest(email).catch(() => void 0);
		return {
			status: "pending",
			email
		};
	...
	if (existing.status !== "Approved" || existing.role === "Unassigned") return {
		status: "pending",
		email
	};
	return {
		...
		email,
		role: existing.role,
		tradeTitle: existing.tradeTitle
	};
}
//#endregion
//#region app/api/staff/status/route.ts
var route_exports$11 = /* @__PURE__ */ __exportAll({ GET: () => GET$8 });
async function GET$8(request) {
	const pending = await verifyPendingStaffSession(cookieValue$1(request, pendingStaffCookieName()));
	...
	if (record.status !== "Approved" || record.role === "Unassigned") return Response.json({
	...
	return Response.json({
		...
		email: record.email,
		role: record.role,
		tradeTitle: record.tradeTitle,
		redirect: staffRedirect(record.role)
	}, { headers });
}
//#endregion
//#region db/task-store.ts
var operationalRoles = new Set(["Site Supervisor", ...workerStaffRoles]);
var managementRoles = new Set(["Admin", "Manager"]);
var allowedPriorities = new Set([
	"Normal",
	"High",
	"Urgent"
]);
var allowedStatuses = new Set([
	"assigned",
	"in_progress",
	"completed"
]);
async function database$1() {
	const { env } = await import("cloudflare:workers");
...
function mapTask(row) {
	return {
		...
		createdByRole: String(row.createdByRole || "Owner"),
		createdByEmail: String(row.createdByEmail || ""),
		createdAt: String(row.createdAt || ""),
		updatedAt: String(row.updatedAt || ""),
		completedAt: String(row.completedAt || "")
...
async function people(db) {
	return (await db.prepare("SELECT email,role,COALESCE(NULLIF(trade_title,''),role) AS title FROM staff_access_requests WHERE status='Approved' AND role!='Unassigned' ORDER BY CASE role WHEN 'Admin' THEN 0 WHEN 'Manager' THEN 1 WHEN 'Site Supervisor' THEN 2 ELSE 3 END,email").all()).results.map((row) => ({
		email: String(row.email).toLowerCase(),
		role: String(row.role),
		title: String(row.title || row.role)
	})).map((row) => ({
		...row,
		group: taskGroup(row.role)
	})).filter((row) => Boolean(row.group));
}
async function projects(db) {
	return (await db.prepare("SELECT id AS caseId,project_code AS projectCode,suburb AS siteLabel FROM workflow_cases WHERE stage='active_project' ORDER BY updated_at DESC").all()).results.map((row) => ({
		caseId: Number(row.caseId),
		projectCode: String(row.projectCode),
		siteLabel: String(row.siteLabel || "Active project")
	}));
}
async function tasks(db, where = "", bindings = []) {
	return (await db.prepare(`SELECT task.id,task.case_id AS caseId,COALESCE(workflow.project_code,'Business / General') AS projectCode,COALESCE(NULLIF(workflow.suburb,''),'Company operations') AS siteLabel,task.assignee_email AS assigneeEmail,task.assignee_role AS assigneeRole,task.assignee_title AS assigneeTitle,task.title,task.instructions,task.priority,task.status,task.created_by_role AS createdByRole,task.created_by_email AS createdByEmail,task.created_at AS createdAt,task.updated_at AS updatedAt,task.completed_at AS completedAt FROM team_tasks task LEFT JOIN workflow_cases workflow ON workflow.id=task.case_id ${where} ORDER BY CASE task.status WHEN 'assigned' THEN 0 WHEN 'in_progress' THEN 1 ELSE 2 END,CASE task.priority WHEN 'Urgent' THEN 0 WHEN 'High' THEN 1 ELSE 2 END,task.updated_at DESC`).bind(...bindings).all()).results.map(mapTask);
}
async function getTaskManagementSnapshot(viewerRole) {
	await ensureTaskDatabase();
	const db = await database$1();
	const [allPeople, allProjects, allTasks] = await Promise.all([
		people(db),
		projects(db),
		tasks(db)
	]);
	...
	return {
		...
		tasks: allTasks.filter((task) => visibleEmails.has(task.assigneeEmail))
	};
}
async function approvedAssignee(db, emailInput) {
	const email = required$1(emailInput, "Assignee").toLowerCase();
	const person = (await people(db)).find((item) => item.email === email);
	if (!person) throw new Error("Choose an approved team member.");
	return person;
}
async function requireTaskScope(db, actorRole, id) {
	const task = await db.prepare("SELECT id,case_id AS caseId,assignee_email AS assigneeEmail,assignee_role AS assigneeRole,title FROM team_tasks WHERE id=? LIMIT 1").bind(id).first();
...
async function taskEvent(db, task, actorRole, actorEmail, eventType, detail) {
	if (!task.caseId) return;
	await db.prepare("INSERT INTO workflow_events (case_id,actor_role,actor_email,event_type,title,detail,audience,created_at) VALUES (?,?,?,?,?,?,'internal',?)").bind(task.caseId, actorRole, actorEmail, eventType, task.title, detail, (/* @__PURE__ */ new Date()).toISOString()).run();
}
async function performTaskManagementAction(actorRole, actorEmail, action, payload) {
	await ensureTaskDatabase();
	const db = await database$1();
	const now = (/* @__PURE__ */ new Date()).toISOString();
	if (action === "create_task") {
		...
		if (caseId) {
			if (!await db.prepare("SELECT id FROM workflow_cases WHERE id=? AND stage='active_project' LIMIT 1").bind(caseId).first()) throw new Error("Choose an active project or Business / General.");
		}
		const title = required$1(payload.title, "Task");
		const instructions = required$1(payload.instructions, "Instructions");
		const priority = clean$1(payload.priority) || "Normal";
		if (!allowedPriorities.has(priority)) throw new Error("Choose a valid priority.");
		await db.prepare("INSERT INTO team_tasks (case_id,assignee_email,assignee_role,assignee_title,title,instructions,priority,status,created_by_role,created_by_email,created_at,updated_at,completed_at) VALUES (?,?,?,?,?,?,?,'assigned',?,?,?,?, '')").bind(caseId, assignee.email, assignee.role, assignee.title, title, instructions, priority, actorRole, actorEmail, now, now).run();
		await taskEvent(db, {
			caseId,
			title
		}, actorRole, actorEmail, "team_task_created", `Assigned to ${assignee.title} (${assignee.email})`);
	} else if (action === "set_status") {
		const id = taskId(payload.taskId);
		const task = await requireTaskScope(db, actorRole, id);
		const status = required$1(payload.status, "Status");
		...
		await taskEvent(db, {
			caseId: Number(task.caseId),
			title: String(task.title)
		}, actorRole, actorEmail, "team_task_status", `Marked ${status.replaceAll("_", " ")}`);
		...
		await taskEvent(db, {
			caseId: Number(task.caseId),
			title: String(task.title)
		}, actorRole, actorEmail, "team_task_deleted", `Removed from ${String(task.assigneeEmail)}`);
	} else throw new Error("Unknown task action.");
}
async function getPersonalTaskSnapshot(emailInput, role) {
	await ensureTaskDatabase();
	const db = await database$1();
	...
	return {
		identity: {
			email,
			role,
			title: (await people(db)).find((item) => item.email === email)?.title || role
		},
		tasks: await tasks(db, "WHERE LOWER(task.assignee_email)=?", [email])
	};
}
async function setPersonalTaskStatus(emailInput, taskIdInput, statusInput) {
	await ensureTaskDatabase();
	const db = await database$1();
	const email = emailInput.toLowerCase();
	const id = taskId(taskIdInput);
	const status = required$1(statusInput, "Status");
	if (!allowedStatuses.has(status)) throw new Error("Choose a valid task status.");
	const task = await db.prepare("SELECT case_id AS caseId,title FROM team_tasks WHERE id=? AND LOWER(assignee_email)=? LIMIT 1").bind(id, email).first();
	...
	await taskEvent(db, {
		caseId: Number(task.caseId),
		title: String(task.title)
	}, "Assignee", email, "team_task_self_status", `Marked ${status.replaceAll("_", " ")}`);
}
//#endregion
//#region app/api/tasks/route.ts
var route_exports$10 = /* @__PURE__ */ __exportAll({
	GET: () => GET$7,
	POST: () => POST$10
});
async function assignee(request) {
...
//#endregion
//#region app/api/tasks/manage/route.ts
var route_exports$9 = /* @__PURE__ */ __exportAll({
	GET: () => GET$6,
	POST: () => POST$9
});
async function manager$1(request) {
	const owner = await ownerSessionFromRequest(request);
	if (owner) return {
		role: "Owner",
		email: owner.email
...
async function GET$6(request) {
	const identity = await manager$1(request);
...
//#endregion
//#region app/api/team/login/route.ts
var route_exports$8 = /* @__PURE__ */ __exportAll({ POST: () => POST$8 });
async function loginKey(email, request) {
	...
	try {
		...
		if (await isRegisteredOwnerEmail(email)) {
			if (!await verifyOwnerPassword(email, password)) {
				...
				await recordLoginFailure(key, failed, failed >= 5 ? now + 900 * 1e3 : 0);
		...
		const result = await processStaffSignIn(email, password, teamCode).catch(() => ({ status: "invalid" }));
		...
		if (result.status === "pending") {
			...
			return Response.json({
				ok: true,
				status: "pending",
				redirect: "/team/pending"
			}, {
				status: 202,
				headers: {
					"Set-Cookie": pendingStaffSessionCookie(token, secure),
					"Cache-Control": "no-store"
				}
			});
		}
		...
		return Response.json({
			ok: true,
			role: result.role,
			redirect: staffRedirect(result.role)
		}, { headers });
	} catch {
...
function melbourneDate(value = /* @__PURE__ */ new Date()) {
	const parts = new Intl.DateTimeFormat("en-AU", {
		timeZone: "Australia/Melbourne",
		year: "numeric",
		month: "2-digit",
...
async function workflowEvent(db, caseId, actorRole, actorEmail, eventType, title, detail = "") {
	await db.prepare("INSERT INTO workflow_events (case_id,actor_role,actor_email,event_type,title,detail,audience,created_at) VALUES (?,?,?,?,?,?,'internal',?)").bind(caseId, actorRole, actorEmail, eventType, title, detail, (/* @__PURE__ */ new Date()).toISOString()).run();
}
async function activeProject(db, caseId) {
	const project = await db.prepare("SELECT id,project_code AS projectCode,suburb,service,project_folder AS projectFolder,stage FROM workflow_cases WHERE id=? LIMIT 1").bind(caseId).first();
...
async function approvedWorker(db, emailInput) {
	const email = required(emailInput, "Worker").toLowerCase();
	const worker = await db.prepare("SELECT email,role,COALESCE(NULLIF(trade_title,''),role) AS tradeTitle FROM staff_access_requests WHERE LOWER(email)=? AND status='Approved' AND role IN ('Worker','Electrician','Plumber','Cleaner','Carpenter','Plasterer','Tiler') LIMIT 1").bind(email).first();
	if (!worker) throw new Error("Choose an approved Worker account.");
	return {
		email,
		role: String(worker.role),
		tradeTitle: String(worker.tradeTitle || worker.role || "Worker")
	};
}
async function markWorkerSignIn(workerEmail) {
	await ensureWorkerDatabase();
	const db = await database();
	const normalizedEmail = workerEmail.toLowerCase();
	if (!await db.prepare("SELECT assignment.id FROM worker_project_assignments assignment JOIN workflow_cases workflow ON workflow.id=assignment.case_id WHERE assignment.worker_email=? AND assignment.status='active' AND workflow.stage='active_project' LIMIT 1").bind(normalizedEmail).first()) return;
	const now = (/* @__PURE__ */ new Date()).toISOString();
	const today = melbourneDate();
	await db.prepare("INSERT INTO worker_attendance (worker_email,work_date,first_seen_at,last_seen_at) VALUES (?,?,?,?) ON CONFLICT(worker_email,work_date) DO UPDATE SET last_seen_at=excluded.last_seen_at").bind(normalizedEmail, today, now, now).run();
}
async function seedWorkerDevelopmentPreview() {}
async function missingDates(db, workerEmail, today) {
	return (await db.prepare("SELECT attendance.work_date AS workDate FROM worker_attendance attendance WHERE attendance.worker_email=? AND attendance.work_date<? AND EXISTS (SELECT 1 FROM worker_project_assignments assignment JOIN workflow_cases workflow ON workflow.id=assignment.case_id WHERE assignment.worker_email=attendance.worker_email AND assignment.status='active' AND workflow.stage='active_project') AND NOT EXISTS (SELECT 1 FROM worker_reports report WHERE report.worker_email=attendance.worker_email AND report.work_date=attendance.work_date) ORDER BY attendance.work_date ASC LIMIT 14").bind(workerEmail, today).all()).results.map((row) => row.workDate);
}
function workerFileRows(files, access) {
...
async function getWorkerSnapshot(workerEmailInput, role) {
	await ensureWorkerDatabase();
	const db = await database();
	...
	const projectIds = new Set(results[0].results.map((item) => Number(item.caseId)));
	const files = results[2].results.filter((file) => projectIds.has(Number(file.caseId))).map((file) => ({
		...file,
		sharedWith: [workerEmail],
		url: `/api/worker/files?id=${file.id}`
	}));
	const profile = results[4].results[0];
	return {
		identity: {
			email: workerEmail,
			role,
			tradeTitle: String(profile?.tradeTitle || role)
		},
		projects: results[0].results,
		tasks: results[1].results.filter((task) => projectIds.has(Number(task.caseId))),
		files,
		reports: results[3].results,
		missingReportDates: await missingDates(db, workerEmail, today),
		today
	};
}
async function submitWorkerReport(workerEmailInput, role, payload) {
	await ensureWorkerDatabase();
	const db = await database();
	const workerEmail = workerEmailInput.toLowerCase();
	...
	if (!await db.prepare("SELECT id FROM worker_attendance WHERE worker_email=? AND work_date=? LIMIT 1").bind(workerEmail, workDate).first()) throw new Error("A report can only be submitted for a recorded Worker sign-in day.");
	const assignment = await db.prepare("SELECT trade_title AS tradeTitle FROM worker_project_assignments WHERE case_id=? AND worker_email=? AND status='active' LIMIT 1").bind(caseId, workerEmail).first();
	...
	const now = (/* @__PURE__ */ new Date()).toISOString();
	await db.prepare("INSERT INTO worker_reports (case_id,worker_email,work_date,completed_work,next_step,issues_delays,status,submitted_at) VALUES (?,?,?,?,?,?,'submitted',?) ON CONFLICT(case_id,worker_email,work_date) DO UPDATE SET completed_work=excluded.completed_work,next_step=excluded.next_step,issues_delays=excluded.issues_delays,status='submitted',submitted_at=excluded.submitted_at,reviewed_by='',reviewed_at='',review_note='' ").bind(caseId, workerEmail, workDate, completedWork, nextStep, issuesDelays, now).run();
...
async function getWorkerManagementSnapshot() {
	await ensureWorkerDatabase();
	const db = await database();
	const results = await db.batch([
	...
	return {
		projects: results[0].results,
		workers: results[1].results,
		assignments: results[2].results,
		tasks: results[3].results,
		files: workerFileRows(results[4].results, results[5].results),
		reports: results[6].results
	};
}
async function performWorkerManagementAction(actorRole, actorEmail, action, payload) {
	await ensureWorkerDatabase();
	const db = await database();
	const now = (/* @__PURE__ */ new Date()).toISOString();
	if (action === "assign_worker") {
		...
		await db.prepare("INSERT INTO worker_project_assignments (case_id,worker_email,trade_title,status,assigned_by,assigned_at,removed_at) VALUES (?,?,?,'active',?,?, '') ON CONFLICT(case_id,worker_email) DO UPDATE SET trade_title=excluded.trade_title,status='active',assigned_by=excluded.assigned_by,assigned_at=excluded.assigned_at,removed_at='' ").bind(caseId, worker.email, tradeTitle, actorEmail, now).run();
		...
		const worker = await approvedWorker(db, payload.workerEmail);
		if (!await db.prepare("SELECT id FROM worker_project_assignments WHERE case_id=? AND worker_email=? AND status='active' LIMIT 1").bind(caseId, worker.email).first()) throw new Error("Assign this Worker to the project before creating a task.");
		const title = required(payload.title, "Task");
		const instructions = required(payload.instructions, "Task instructions");
		await db.prepare("INSERT INTO worker_tasks (case_id,worker_email,title,instructions,status,created_by,created_at,updated_at) VALUES (?,?,?,?,'assigned',?,?,?)").bind(caseId, worker.email, title, instructions, actorEmail, now, now).run();
		...
		if (![
			...
			"in_progress",
			"completed"
		].includes(status)) throw new Error("Choose a valid task status.");
		const task = await db.prepare("SELECT case_id AS caseId,title FROM worker_tasks WHERE id=? LIMIT 1").bind(taskId).first();
		if (!task) throw new Error("Task was not found.");
		...
		const file = await db.prepare("SELECT id,case_id AS caseId,file_name AS fileName FROM workflow_files WHERE id=? AND category='worker_document' LIMIT 1").bind(fileId).first();
		...
		if (granted) await db.prepare("INSERT INTO worker_file_access (file_id,case_id,worker_email,granted_by,granted_at) VALUES (?,?,?,?,?) ON CONFLICT(file_id,worker_email) DO UPDATE SET granted_by=excluded.granted_by,granted_at=excluded.granted_at").bind(fileId, Number(file.caseId), worker.email, actorEmail, now).run();
		...
		const report = await db.prepare("SELECT case_id AS caseId FROM worker_reports WHERE id=? LIMIT 1").bind(reportId).first();
	...
	} else throw new Error("Unknown Worker management action.");
}
async function addWorkerProjectFile(caseIdInput, file, workerEmails, actorRole, actorEmail) {
	await ensureWorkerDatabase();
	const db = await database();
	...
	for (const email of uniqueWorkers) {
		...
		if (!await db.prepare("SELECT id FROM worker_project_assignments WHERE case_id=? AND worker_email=? AND status='active' LIMIT 1").bind(caseId, email).first()) throw new Error(`${email} must be assigned to this project before receiving a file.`);
	}
	const { env } = await import("cloudflare:workers");
	...
	const objectKey = `${String(project.projectFolder || `projects/${project.projectCode}`)}/worker-files/${crypto.randomUUID()}-${safeName}`;
	...
	const now = (/* @__PURE__ */ new Date()).toISOString();
	try {
		const created = await db.prepare("INSERT INTO workflow_files (case_id,category,object_key,file_name,mime_type,size_bytes,uploaded_by,visibility,uploaded_at) VALUES (?,'worker_document',?,?,?,?,?,'internal',?)").bind(caseId, objectKey, file.name, file.type || "application/octet-stream", file.size, actorEmail, now).run();
...
async function getWorkerFile(fileId, workerEmail, management = false) {
	await ensureWorkerDatabase();
	const db = await database();
	if (management) return db.prepare("SELECT object_key AS objectKey,mime_type AS mimeType,file_name AS fileName FROM workflow_files WHERE id=? AND category='worker_document' LIMIT 1").bind(fileId).first();
	if (!workerEmail) return null;
	return db.prepare("SELECT file.object_key AS objectKey,file.mime_type AS mimeType,file.file_name AS fileName FROM workflow_files file JOIN worker_file_access access ON access.file_id=file.id JOIN worker_project_assignments assignment ON assignment.case_id=access.case_id AND assignment.worker_email=access.worker_email AND assignment.status='active' WHERE file.id=? AND file.category='worker_document' AND access.worker_email=? LIMIT 1").bind(fileId, workerEmail.toLowerCase()).first();
}
//#endregion
//#region app/api/worker/route.ts
var route_exports$7 = /* @__PURE__ */ __exportAll({
	GET: () => GET$5,
	POST: () => POST$7
});
async function worker$1(request) {
	const session = await adminSessionFromRequest(request);
	if (session && isWorkerRole(session.role)) return session;
	return null;
}
async function GET$5(request) {
	const session = await worker$1(request);
...
//#endregion
//#region app/api/worker/files/route.ts
...
async function identity(request) {
	...
	if (staff && isWorkerRole(staff.role)) return {
		management: false,
		role: staff.role,
		email: staff.email
	};
...
async function POST$6(request) {
	...
	try {
		...
		const workerEmails = JSON.parse(String(form.get("workerEmails") ?? "[]"));
		...
		await addWorkerProjectFile(caseId, file, workerEmails, actor.role, actor.email);
...
//#endregion
//#region app/api/worker/manage/route.ts
...
//#endregion
//#region app/api/workflow/route.ts
var route_exports$4 = /* @__PURE__ */ __exportAll({
	GET: () => GET$2,
	POST: () => POST$4
});
async function actor$1(request) {
	const owner = await ownerSessionFromRequest(request);
	if (owner) return {
		role: "owner",
		email: owner.email
...
async function GET$2(request) {
	const identity = await actor$1(request);
...
//#endregion
//#region app/api/workflow/files/route.ts
var route_exports$3 = /* @__PURE__ */ __exportAll({
	GET: () => GET$1,
	POST: () => POST$3
});
async function imageSignatureMatches(file) {
	const bytes = new Uint8Array(await file.slice(0, 16).arrayBuffer());
	if (file.type === "image/jpeg") return bytes[0] === 255 && bytes[1] === 216 && bytes[2] === 255;
	if (file.type === "image/png") return bytes.slice(0, 8).every((byte, index) => byte === [
		137,
		80,
		78,
		71,
...
async function POST$3(request) {
	...
	try {
		...
		if (!(file instanceof File) || !Number.isInteger(caseId) || ![
			"site_visit",
			"progress",
			"quality",
			"document"
		].includes(category)) throw new Error("Choose a valid project photo.");
		...
		if (!(category === "document" ? new Set([...safeImageTypes, "application/pdf"]) : safeImageTypes).has(file.type.toLowerCase())) throw new Error(category === "document" ? "Only JPEG, PNG, WebP, HEIC or PDF files are accepted." : "Only JPEG, PNG, WebP or HEIC site photos are accepted.");
		...
		const item = (await getWorkflowSnapshot(identity.role, identity.email)).cases.find((entry) => entry.id === caseId);
		...
		const objectKey = `${item.projectFolder || `requests/${item.requestCode}`}/photos/${category}/${crypto.randomUUID()}-${safeName}`;
		await env.BUCKET.put(objectKey, file.stream(), {
...
//#endregion
//#region app/api/workflow/public/route.ts
var route_exports$2 = /* @__PURE__ */ __exportAll({
	GET: () => GET,
	PATCH: () => PATCH,
	POST: () => POST$2
});
async function throttleKey(request, purpose) {
...
async function blocked(key) {
	...
	return {
		attempt,
		isBlocked: Boolean(attempt && attempt.lockedUntil > Date.now())
	};
}
function failed(error) {
	const message = error instanceof Error ? error.message : "";
	const safe = /(?:required|too long|valid email|accept the Privacy Notice|current project stage|project was not found)/i.test(message) ? message : "The request could not be completed.";
...
async function GET(request) {
	...
	if (!data) {
		...
		await recordLoginFailure(key, count, count >= 12 ? Date.now() + 1800 * 1e3 : 0);
...
async function POST$2(request) {
	...
	try {
		...
		const created = await createPublicWorkflowRequest(await request.json());
		const count = (limit.attempt?.failedCount ?? 0) + 1;
		await recordLoginFailure(key, count, count >= 5 ? Date.now() + 3600 * 1e3 : 0);
...
//#endregion
//#region app/api/workflow/public/access/route.ts
var route_exports$1 = /* @__PURE__ */ __exportAll({ POST: () => POST$1 });
async function requestKey(request, kind) {
...
async function POST$1(request) {
	...
	if (!(kind === "email" ? /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(contact) && contact.length <= 254 : /^0\d{9}$/.test(contact))) return Response.json({ error: kind === "email" ? "Enter a valid email address." : "Enter a valid Australian phone number." }, { status: 400 });
	...
	if (!projects.length) {
		...
		await recordLoginFailure(key, count, count >= 5 ? Date.now() + 3600 * 1e3 : 0);
		return Response.json({ error: "No project matches that saved email or phone number." }, {
			status: 404,
			headers: { "Cache-Control": "no-store" }
		});
	}
...
//#endregion
//#region app/api/workflow/public/files/route.ts
...
async function POST(request) {
	...
	try {
		...
		objectKey = `${target.projectFolder || `requests/${target.requestCode}`}/customer-intake/${crypto.randomUUID()}-${safeName}`;
		await env.BUCKET.put(objectKey, file.stream(), {
			httpMetadata: { contentType: file.type },
			customMetadata: {
				caseId: String(caseId),
				uploadedBy: target.customerEmail,
				originalName: file.name
			}
		});
		...
		if (objectKey) {
			const { env } = await import("cloudflare:workers");
			await env.BUCKET?.delete(objectKey).catch(() => void 0);
		}
		const message = error instanceof Error ? error.message : "Customer document upload failed.";
		const safe = /(?:valid customer document|only JPG|5 MB|file content|not found|maximum|no longer|not configured)/i.test(message) ? message : "Customer document upload failed.";
		return Response.json({ error: safe }, { status: 400 });
	}
}
//#endregion
//#region app/owner/OwnerDashboard.tsx
var OwnerDashboard_default = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'default' is called on server");
}, "07e5d253561f", "default");
//#endregion
//#region app/owner/page.tsx
var page_exports$5 = /* @__PURE__ */ __exportAll({
	default: () => OwnerPage,
	dynamic: () => dynamic$4
});
var dynamic$4 = "force-dynamic";
async function OwnerPage({ searchParams }) {
	await searchParams;
	const session = await verifyOwnerSession((await cookies()).get(ownerCookieName())?.value);
	if (!session) redirect("/#team-sign-in");
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(OwnerDashboard_default, {
		ownerName: await ownerDisplayName(session.email),
...
var SiteSupervisor_default = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'default' is called on server");
}, "a9766214951c", "default");
//#endregion
//#region app/site-supervisor/page.tsx
var page_exports$4 = /* @__PURE__ */ __exportAll({
	default: () => SiteSupervisorPage,
	dynamic: () => dynamic$3
});
var dynamic$3 = "force-dynamic";
async function SiteSupervisorPage({ searchParams }) {
	await searchParams;
	if ((await verifyAdminSession((await cookies()).get(adminCookieName())?.value))?.role === "Site Supervisor") return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(SiteSupervisor_default, {});
		...
		children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("section", {
			...
			children: [
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
					...
					children: "Role-protected workspace"
				}),
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h1", {
					...
					children: "Site Supervisor access"
				}),
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
					style: {
						color: "#a4a7a7",
					...
					children: "This workspace opens only after the Owner approves the Site Supervisor role. Sign in through Team Sign In to continue."
				}),
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(link_default, {
					...
					children: "Return to Team Sign In"
				})
			]
		})
	});
...
var PendingAccess_default = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'default' is called on server");
}, "03672e79ab9b", "default");
//#endregion
//#region app/team/pending/page.tsx
var page_exports$3 = /* @__PURE__ */ __exportAll({
	default: () => PendingStaffPage,
	dynamic: () => dynamic$2
});
var dynamic$2 = "force-dynamic";
async function PendingStaffPage({ searchParams }) {
	await searchParams;
	const pending = await verifyPendingStaffSession((await cookies()).get(pendingStaffCookieName())?.value);
	if (!pending) redirect("/");
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(PendingAccess_default, { email: pending.email });
}
...
})(import_react_react_server.default, assetsManifest.serverResources["app/team/workspace/page.tsx"], void 0, "vite-rsc/importer-resources");
//#endregion
...
var TaskInbox_default = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'default' is called on server");
}, "7546c08984d7", "default");
...
//#endregion
//#region app/team/workspace/page.tsx
var page_exports$2 = /* @__PURE__ */ __exportAll({
	default: () => $$wrap_TeamWorkspacePage,
	dynamic: () => dynamic$1
});
var dynamic$1 = "force-dynamic";
async function TeamWorkspacePage() {
	const session = await verifyAdminSession((await cookies()).get(adminCookieName())?.value);
	if (!session) redirect("/");
	...
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("main", {
		...
		children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("section", {
			...
			children: [
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(BrandLogo, {
					kind: "tradie",
					tone: "dark",
				...
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("div", {
					...
					children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", { children: "✓" })
				}),
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
					className: team_module_default.eyebrow,
					children: "Access approved"
				}),
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("h1", { children: [session.role, " workspace assigned."] }),
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", { children: "Your identity and role are active. Tasks assigned to you by Owner appear below and can be started or completed from this workspace." }),
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
					className: team_module_default.identityRow,
					children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", { children: "Approved account" }), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("strong", { children: session.email })]
				}),
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(TaskInbox_default, { role: session.role }),
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(link_default, {
					href: "/",
...
var $$wrap_TeamWorkspacePage = /* @__PURE__ */ __vite_rsc_wrap_css__(TeamWorkspacePage, "default");
...
var WorkerDashboard_default = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'default' is called on server");
}, "d74b842818c6", "default");
//#endregion
//#region app/worker/page.tsx
var page_exports$1 = /* @__PURE__ */ __exportAll({
	default: () => WorkerPage,
	dynamic: () => dynamic
});
var dynamic = "force-dynamic";
async function WorkerPage({ searchParams }) {
	await searchParams;
	const session = await verifyAdminSession((await cookies()).get(adminCookieName())?.value);
	if (session && isWorkerRole(session.role)) return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(WorkerDashboard_default, {});
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("main", {
		style: {
			minHeight: "100vh",
			display: "grid",
		...
		children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("section", {
			...
			children: [
				...
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
					...
					children: "This page opens only after Owner approval assigns a Worker or trade role to your Team Code account."
				}),
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(link_default, {
					href: "/#team-sign-in",
					style: {
...
var routes = [
	...
	},
	{
		...
		ids: {
			"route": "route:/api/admin/data",
			"page": null,
			"routeHandler": "route-handler:/api/admin/data",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
	...
	},
	{
		...
		ids: {
			"route": "route:/api/operations",
			"page": null,
			"routeHandler": "route-handler:/api/operations",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
	...
	},
	{
		...
		ids: {
			"route": "route:/api/owner/data",
			"page": null,
			"routeHandler": "route-handler:/api/owner/data",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/owner/data",
		patternParts: [
			"api",
			"owner",
			"data"
	...
	},
	{
		...
		ids: {
			"route": "route:/api/owner/logout",
			"page": null,
			"routeHandler": "route-handler:/api/owner/logout",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/owner/logout",
		patternParts: [
			"api",
			"owner",
			"logout"
	...
	},
	{
		...
		ids: {
			"route": "route:/api/owner/settings",
			"page": null,
			"routeHandler": "route-handler:/api/owner/settings",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/owner/settings",
		patternParts: [
			"api",
			"owner",
			"settings"
	...
	},
	{
		...
		ids: {
			"route": "route:/api/staff/status",
			"page": null,
			"routeHandler": "route-handler:/api/staff/status",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/staff/status",
		patternParts: [
			"api",
			"staff",
			"status"
	...
	},
	{
		...
		ids: {
			"route": "route:/api/tasks",
			"page": null,
			"routeHandler": "route-handler:/api/tasks",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
	...
	},
	{
		...
		ids: {
			"route": "route:/api/tasks/manage",
			"page": null,
			"routeHandler": "route-handler:/api/tasks/manage",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/tasks/manage",
		patternParts: [
			"api",
			"tasks",
			"manage"
	...
	},
	{
		...
		ids: {
			"route": "route:/api/team/login",
			"page": null,
			"routeHandler": "route-handler:/api/team/login",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/team/login",
		patternParts: [
			"api",
			"team",
			"login"
	...
	},
	{
		...
		ids: {
			"route": "route:/api/worker",
			"page": null,
			"routeHandler": "route-handler:/api/worker",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
	...
	},
	{
		...
		ids: {
			"route": "route:/api/worker/files",
			"page": null,
			"routeHandler": "route-handler:/api/worker/files",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/worker/files",
		patternParts: [
			"api",
			"worker",
			"files"
	...
	},
	{
		...
		ids: {
			"route": "route:/api/worker/manage",
			"page": null,
			"routeHandler": "route-handler:/api/worker/manage",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/worker/manage",
		patternParts: [
			"api",
			"worker",
			"manage"
	...
	},
	{
		...
		ids: {
			"route": "route:/api/workflow",
			"page": null,
			"routeHandler": "route-handler:/api/workflow",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
	...
	},
	{
		...
		ids: {
			"route": "route:/api/workflow/files",
			"page": null,
			"routeHandler": "route-handler:/api/workflow/files",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/workflow/files",
		patternParts: [
			"api",
			"workflow",
			"files"
	...
	},
	{
		...
		ids: {
			"route": "route:/api/workflow/public",
			"page": null,
			"routeHandler": "route-handler:/api/workflow/public",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/workflow/public",
		patternParts: [
			"api",
			"workflow",
			"public"
	...
	},
	{
		...
		ids: {
			"route": "route:/api/workflow/public/access",
			"page": null,
			"routeHandler": "route-handler:/api/workflow/public/access",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/workflow/public/access",
		patternParts: [
			"api",
			"workflow",
			"public",
	...
	},
	{
		...
		ids: {
			"route": "route:/api/workflow/public/files",
			"page": null,
			"routeHandler": "route-handler:/api/workflow/public/files",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/workflow/public/files",
		patternParts: [
			"api",
			"workflow",
			"public",
	...
	},
	{
		...
		ids: {
			"route": "route:/site-supervisor",
			"page": "page:/site-supervisor",
			"routeHandler": null,
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
		...
		pattern: "/site-supervisor",
		patternParts: ["site-supervisor"],
		isDynamic: false,
		params: [],
		rootParamNames: [],
		page: page_exports$4,
	...
	},
	{
		...
		ids: {
			"route": "route:/team/pending",
			"page": "page:/team/pending",
			"routeHandler": null,
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
		...
		pattern: "/team/pending",
		patternParts: ["team", "pending"],
		isDynamic: false,
		params: [],
		rootParamNames: [],
		page: page_exports$3,
	...
	},
	{
		...
		ids: {
			"route": "route:/team/workspace",
			"page": "page:/team/workspace",
			"routeHandler": null,
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
	...
	},
	{
		...
		ids: {
			"route": "route:/track/:code",
			"page": "page:/track/:code",
			"routeHandler": null,
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
...
var __publicFiles = new Set([
	...
	"/images/hero-complete.webp",
	"/images/hero-construction.webp",
	"/images/logo-alert-3d-pro.svg",
	"/images/logo-alert-construction-mark.png",
	"/images/logo-alert-construction.png",
	"/images/logo-alert-construction.webp",
	"/images/logo-alert-engineers-mark.png",
	"/images/logo-alert-engineers.png",
	"/images/logo-alert-engineers.webp",
	"/images/logo-alert-tradie-pro-dark.svg",
	"/images/logo-alert-tradie-pro-mark-v53.png",
	"/images/logo-alert-tradie-pro.svg",
	"/images/logo-alert-tradie-pro.webp",
	"/images/service-bathroom.webp",
	"/images/service-engineering.webp",
	"/images/service-extension.webp",
	"/images/service-inspection.webp",
	"/images/service-kitchen.webp",
	"/images/service-maintenance.webp",
	"/images/service-new-home.webp",
	"/images/service-renovation.webp",
	"/window.svg"
]);
...
var _virtual_vinext_rsc_entry_default = createAppRscHandler({
	...
	setNavigationContext: setAppNavigationContext,
	staticParamsMap: { "/track/:code": null },
...
function securityHeaders(response, url) {
	const headers = new Headers(response.headers);
	headers.set("Content-Security-Policy", "default-src 'self'; base-uri 'self'; object-src 'none'; frame-ancestors 'none'; form-action 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:; font-src 'self' data:; connect-src 'self'; media-src 'self'; worker-src 'self' blob:");
	...
	if (/^\/(?:api|owner|admin|site-supervisor|worker|team|track)(?:\/|$)/.test(url.pathname)) {
...
var worker_entry_default = { async fetch(request, env, ctx) {
	const url = new URL(request.url);
	if (url.pathname === "/_vinext/image") return securityHeaders(await handleImageOptimization(request, {
	...
	}, [...DEFAULT_DEVICE_SIZES, ...DEFAULT_IMAGE_SIZES]), url);
	return securityHeaders(await app_router_entry_default.fetch(request, env, ctx), url);
} };
//#endregion
