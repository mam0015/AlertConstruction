globalThis.__VINEXT_LAZY_CHUNKS__ = ["assets/BrandLogo-Bap3h6LK.js","assets/admin.module-CaLcNX6v.js","assets/owner.module-fHZFQeqZ.js","assets/AdminDashboard-VN0PN9jI.js","assets/AdminLogin-jbRDOx9D.js","assets/OwnerDashboard-Dm619yMv.js","assets/OwnerLogin-CHQnZEKA.js","assets/page-D0S34b1a.js","assets/PendingAccess-B9GUZhK4.js","assets/page-B82Pmoa8.js","assets/layout-segment-context-DE3qhgT8.js","assets/link-BK4FlEzJ.js","assets/router-CPoFnZ-m.js"];
...
function getDraftSecret() {
	return "7a23f0ce-69f9-4678-9e0d-0c1380f2de03";
}
/**
...
var NextURL = class NextURL {
	...
	*/
	get buildId() {
		return "80b6a389-8d61-4790-af9e-822089b38686";
	}
};
...
*/
function appIsrCacheKey(pathname, suffix, buildId = "80b6a389-8d61-4790-af9e-822089b38686") {
...
function createAppPageArtifactCompatibility(element, routePattern) {
	...
	return createArtifactCompatibilityEnvelope({
		...
		deploymentVersion: "80b6a389-8d61-4790-af9e-822089b38686",
		rootBoundaryId
	});
}
/**
...
async function ensureStaffAccessTable() {
	await (await database$3()).prepare(`CREATE TABLE IF NOT EXISTS staff_access_requests (
...
async function getStaffAccessRequest(email) {
	...
	return (await database$3()).prepare(`SELECT id,email,password_hash AS passwordHash,status,role,trade_title AS tradeTitle,
...
async function listStaffAccessRequests() {
	await ensureStaffAccessTable();
	return (await (await database$3()).prepare(`SELECT id,email,status,role,trade_title AS tradeTitle,
    requested_at AS requestedAt,reviewed_at AS reviewedAt,last_seen_at AS lastSeenAt
...
async function touchStaffAccessRequest(email) {
	await ensureStaffAccessTable();
	await (await database$3()).prepare("UPDATE staff_access_requests SET last_seen_at=? WHERE email=?").bind((/* @__PURE__ */ new Date()).toISOString(), email.toLowerCase()).run();
}
...
var ADMIN_COOKIE = "atp_admin_session";
...
async function verifyStoredSecret(value, storedHash) {
	const [algorithm, iterationsText, saltText, expectedText] = storedHash.split("$");
	const iterations = Number(iterationsText);
	if (algorithm !== "pbkdf2" || iterations !== 1e5 || !saltText || !expectedText) return false;
	const key = await crypto.subtle.importKey("raw", encoder$1.encode(value), "PBKDF2", false, ["deriveBits"]);
	const expected = base64UrlDecode$1(expectedText);
	return safeEqual$1(new Uint8Array(await crypto.subtle.deriveBits({
		name: "PBKDF2",
		hash: "SHA-256",
		salt: base64UrlDecode$1(saltText),
		iterations
	}, key, expected.length * 8)), expected);
}
async function hashStaffPassword(value) {
	...
	return `pbkdf2$100000$${base64UrlEncode$1(salt)}$${base64UrlEncode$1(bits)}`;
}
function verifyCompanyTeamCode(teamCode) {
	return verifyStoredSecret(teamCode.trim().toUpperCase(), required("ADMIN_TEAM_CODE_HASH"));
}
async function hmac$1(value) {
	const key = await crypto.subtle.importKey("raw", encoder$1.encode(required("ADMIN_SESSION_SECRET")), {
		name: "HMAC",
		hash: "SHA-256"
	}, false, ["sign"]);
	return new Uint8Array(await crypto.subtle.sign("HMAC", key, encoder$1.encode(value)));
}
async function createAdminSession(email, role = "Admin") {
...
async function verifyAdminSession(token) {
	...
	if (!safeEqual$1(await hmac$1(payload), base64UrlDecode$1(signatureText))) return null;
	try {
...
function adminSessionCookie(token, secure) {
	return `${ADMIN_COOKIE}=${token}; Path=/; HttpOnly; SameSite=Strict; Max-Age=${SESSION_SECONDS$1}${secure ? "; Secure" : ""}`;
}
...
async function adminSessionFromRequest(request) {
	return verifyAdminSession(cookieValue$1(request, ADMIN_COOKIE));
}
//#endregion
...
async function seedOwnerDatabase() {
	...
	const counts = await db.batch([
		...
		db.prepare("SELECT COUNT(*) AS total FROM finance_entries"),
		db.prepare("SELECT COUNT(*) AS total FROM schedule_events"),
		db.prepare("SELECT COUNT(*) AS total FROM eod_reports"),
		db.prepare("SELECT COUNT(*) AS total FROM team_messages")
	]);
	const total = (i) => Number(counts[i].results[0]?.total ?? 0);
	const statements = [];
	const now = (/* @__PURE__ */ new Date()).toISOString();
	if (!total(0)) for (const row of [
		[
			"ATP-2026-00124",
		...
		[
			...
			"Rowville",
			"2026-08-03",
			"Engineering inspection and report."
		],
		[
			"ATP-2026-00136",
			"Bathroom renovation 136",
			"Bathroom Renovation",
			"Estimate",
			18,
			428e4,
			...
			"Wantirna",
			"2026-08-12",
			"Scope and supplier pricing in review."
		],
		[
			"ATP-2026-00141",
			"Home extension 141",
			...
			"Wheelers Hill",
			"",
			"New request awaiting owner review."
		]
	]) statements.push(db.prepare("INSERT INTO projects (code,name,service,stage,progress,contract_value,balance,customer_name,suburb,start_date,notes,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)").bind(...row, now));
	if (!total(1)) for (const row of [
		[
			"Income",
			"Progress payment",
			"ATP-2026-00124",
	...
	]) statements.push(db.prepare("INSERT INTO finance_entries (type,category,project_code,amount,entry_date,note,created_at) VALUES (?,?,?,?,?,?,?)").bind(...row, now));
	if (!total(2)) for (const row of [
		[
			"2026-08-10",
			"09:00",
	...
	]) statements.push(db.prepare("INSERT INTO schedule_events (event_date,start_time,title,assignee,project_code,tone,notes) VALUES (?,?,?,?,?,?,?)").bind(...row));
	if (!total(3)) for (const row of [
		[
			"Site Supervisor 01",
			"Site Supervisor",
			"ATP-2026-00124",
			"Checked framing dimensions, uploaded 8 site photos and confirmed plumbing set-out.",
			"2026-08-09T16:42:00+10:00",
			"Pending",
			""
		],
		[
			...
			"Admin",
			"ATP-2026-00136",
			"Reviewed customer brief, prepared scope notes and requested two supplier prices.",
			"2026-08-09T16:18:00+10:00",
			"Pending",
			""
		],
		[
			...
			"Worker",
			"ATP-2026-00124",
			"Completed demolition clean-up and separated waste for collection.",
			"2026-08-09T16:05:00+10:00",
			"Approved",
			"Good work. Keep the waste docket with the project."
		]
	]) statements.push(db.prepare("INSERT INTO eod_reports (person,role,project_code,summary,submitted_at,status,owner_note) VALUES (?,?,?,?,?,?,?)").bind(...row));
	if (!total(4)) for (const row of [
		[
			"Site Supervisor 01",
			"Owner",
			"Morning Ali, the framing dimensions are checked and the plumbing set-out is ready.",
			"2026-08-09T10:36:00+10:00"
		],
		[
			"Owner",
			"Site Supervisor 01",
			"Great. Please upload the marked-up photos to Project 124 before the EOD report.",
			"2026-08-09T10:40:00+10:00"
		],
		[
			"Site Supervisor 01",
			"Owner",
			"Done — 8 photos are now attached to the project.",
			"2026-08-09T10:44:00+10:00"
		]
	]) statements.push(db.prepare("INSERT INTO team_messages (sender,recipient,body,sent_at) VALUES (?,?,?,?)").bind(...row));
	statements.push(db.prepare("INSERT OR IGNORE INTO role_permissions (role,projects,schedule,finance,finance_export,updated_at) VALUES ('Admin',1,1,0,0,?)").bind(now));
	if (statements.length) await db.batch(statements);
}
async function getOwnerSnapshot() {
	await ensureOwnerDatabase();
	await seedOwnerDatabase();
	const db = await database$2();
...
async function createOwnerRecord(resource, p) {
	await ensureOwnerDatabase();
	const db = await database$2(), now = (/* @__PURE__ */ new Date()).toISOString();
...
async function deleteOwnerRecord(resource, id) {
	...
	await (await database$2()).prepare(`DELETE FROM ${table} WHERE id=?`).bind(id).run();
}
async function getLoginAttempt(key) {
	await ensureOwnerDatabase();
	return (await database$2()).prepare("SELECT failed_count AS failedCount,locked_until AS lockedUntil FROM login_attempts WHERE key=?").bind(key).first();
}
async function recordLoginFailure(key, failedCount, lockedUntil) {
	await ensureOwnerDatabase();
	await (await database$2()).prepare("INSERT INTO login_attempts (key,failed_count,locked_until,updated_at) VALUES (?,?,?,?) ON CONFLICT(key) DO UPDATE SET failed_count=excluded.failed_count,locked_until=excluded.locked_until,updated_at=excluded.updated_at").bind(key, failedCount, lockedUntil, Date.now()).run();
}
async function clearLoginFailures(key) {
	await ensureOwnerDatabase();
	await (await database$2()).prepare("DELETE FROM login_attempts WHERE key=?").bind(key).run();
}
//#endregion
//#region db/request-store.ts
async function database$1() {
	const env = await runtimeBindings();
	if (!env.DB) throw new Error("The request database is unavailable.");
	return env.DB;
}
async function ensureRequestFilesTable() {
	await ensureOwnerDatabase();
...
function requestCode() {
	const random = crypto.getRandomValues(new Uint32Array(1))[0] % 1e5;
	return `ATP-${(/* @__PURE__ */ new Date()).getUTCFullYear()}-${String(random).padStart(5, "0")}`;
}
async function uniqueRequestCode() {
	const db = await database$1();
	for (let attempt = 0; attempt < 12; attempt += 1) {
		const code = requestCode();
		if (!await db.prepare("SELECT code FROM job_requests WHERE code=? UNION SELECT code FROM projects WHERE code=? LIMIT 1").bind(code, code).first()) return code;
	}
	return `ATP-${(/* @__PURE__ */ new Date()).getUTCFullYear()}-${crypto.randomUUID().slice(0, 8).toUpperCase()}`;
}
async function createPublicRequest(input, files) {
	...
	try {
      ...
      VALUES (?,?,?,?,?,?,?,'New','Normal',?,'Unassigned',?)`).bind(code, "Job Request", input.customerName, `${input.phone} · ${input.email}`, service, input.location, now, summary, now).run();
		if (files.length && env.BUCKET) for (const file of files) {
          ...
          VALUES (?,?,?,?,?,?)`).bind(code, objectKey, file.name, file.type || "application/octet-stream", file.size, now).run();
		}
	} catch (error) {
		if (env.BUCKET) await Promise.all(uploadedKeys.map((key) => env.BUCKET.delete(key).catch(() => void 0)));
		await db.prepare("DELETE FROM request_files WHERE request_code=?").bind(code).run().catch(() => void 0);
		await db.prepare("DELETE FROM job_requests WHERE code=?").bind(code).run().catch(() => void 0);
		throw error;
	}
	return {
		code,
...
async function getTrackingRecord(codeInput) {
	await ensureRequestFilesTable();
    ...
    FROM projects WHERE code=? LIMIT 1`).bind(code).first();
	if (project) return {
		...project,
		recordType: "project",
		attachmentCount: 0
    ...
    FROM job_requests WHERE code=? LIMIT 1`).bind(code).first();
	if (!request) return null;
	return {
		...request,
		recordType: "request",
		progress: request.stage === "New" ? 5 : request.stage === "Contacted" ? 12 : 18
	};
}
async function createCustomerMessage(codeInput, subject, message) {
	const record = await getTrackingRecord(codeInput);
	if (!record) throw new Error("Project reference not found.");
	await (await database$1()).prepare("INSERT INTO team_messages (sender,recipient,body,sent_at) VALUES (?,?,?,?)").bind(`Customer · ${record.code}`, "Admin", `${subject.trim()}\n${message.trim()}`, (/* @__PURE__ */ new Date()).toISOString()).run();
}
//#endregion
//#region app/owner-auth.ts
var OWNER_COOKIE = "atp_owner_session";
var SESSION_SECONDS = 3600 * 8;
...
async function hmac(value) {
	const key = await crypto.subtle.importKey("raw", encoder.encode(requiredEnvironmentValue("OWNER_SESSION_SECRET")), {
		name: "HMAC",
		hash: "SHA-256"
	}, false, ["sign"]);
	return new Uint8Array(await crypto.subtle.sign("HMAC", key, encoder.encode(value)));
}
function registeredOwnerEmail() {
	return requiredEnvironmentValue("OWNER_EMAIL").toLowerCase();
}
async function verifyOwnerPassword(password) {
	const [algorithm, iterationsText, saltText, expectedText] = requiredEnvironmentValue("OWNER_PASSWORD_HASH").split("$");
	const iterations = Number(iterationsText);
	if (algorithm !== "pbkdf2" || iterations !== 1e5 || !saltText || !expectedText) return false;
...
async function verifyOwnerSession(token) {
	...
	try {
		...
		if (!parsed.email || !parsed.expires || parsed.expires < Date.now() || parsed.email.toLowerCase() !== registeredOwnerEmail()) return null;
		return {
			email: parsed.email.toLowerCase(),
			expires: parsed.expires
		};
...
//#endregion
//#region app/api/track/[code]/messages/route.ts
...
async function POST$8(request, context) {
	...
	const message = payload.message?.trim().slice(0, 4e3) ?? "";
...
//#endregion
//#region app/page.tsx
var page_exports$5 = /* @__PURE__ */ __exportAll({ default: () => page_default$1 });
...
var AdminLogin_default = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'default' is called on server");
}, "3618e549b399", "default");
//#endregion
...
async function AdminPage() {
	...
	if (owner) return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(AdminDashboard_default, {
		viewerName: "Ali Mobini",
		viewerEmail: owner.email,
		previewAsOwner: true
	});
	const admin = await verifyAdminSession(cookieStore.get(adminCookieName())?.value);
	if (!admin || admin.role !== "Admin") return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(AdminLogin_default, {});
...
async function adminPermissions() {
	return await (await database()).prepare("SELECT role,projects,schedule,finance,finance_export AS financeExport FROM role_permissions WHERE role='Admin' LIMIT 1").first() ?? {
		role: "Admin",
		projects: 1,
		schedule: 1,
		finance: 0,
...
async function createAdminRecord(resource, payload) {
	...
	const now = (/* @__PURE__ */ new Date()).toISOString();
	if (resource === "project") {
		await db.prepare("INSERT INTO projects (code,name,service,stage,progress,contract_value,balance,customer_name,suburb,start_date,notes,updated_at) VALUES (?,?,?,?,?,0,0,?,?,?,?,?)").bind(text$1(payload.code).toUpperCase(), text$1(payload.name), text$1(payload.service), text$1(payload.stage, "Admin review"), Math.min(100, Math.max(0, number(payload.progress))), text$1(payload.customerName), text$1(payload.suburb), text$1(payload.startDate), text$1(payload.notes), now).run();
		return;
	}
	if (resource === "request") {
		await db.prepare("INSERT INTO job_requests (code,request_type,customer_name,contact,service,suburb,submitted_at,status,priority,summary,assigned_to,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)").bind(text$1(payload.code).toUpperCase(), text$1(payload.requestType, "Job Request"), text$1(payload.customerName), text$1(payload.contact), text$1(payload.service), text$1(payload.suburb), text$1(payload.submittedAt, now), text$1(payload.status, "New"), text$1(payload.priority, "Normal"), text$1(payload.summary), text$1(payload.assignedTo, "Unassigned"), now).run();
		return;
	}
	if (resource === "schedule") {
		await db.prepare("INSERT INTO schedule_events (event_date,start_time,title,assignee,project_code,tone,notes) VALUES (?,?,?,?,?,?,?)").bind(text$1(payload.eventDate), text$1(payload.startTime), text$1(payload.title), text$1(payload.assignee, "Site Supervisor 01"), text$1(payload.projectCode, "Business / General"), text$1(payload.tone, "gold"), text$1(payload.notes)).run();
		return;
	}
	if (resource === "message") {
		await db.prepare("INSERT INTO team_messages (sender,recipient,body,sent_at) VALUES (?,?,?,?)").bind("Admin", text$1(payload.recipient, "Site Supervisor 01"), text$1(payload.body), now).run();
		return;
	}
}
async function updateAdminRecord(resource, id, payload) {
	...
	const now = (/* @__PURE__ */ new Date()).toISOString();
	if (resource === "project") {
		await db.prepare("UPDATE projects SET code=?,name=?,service=?,stage=?,progress=?,customer_name=?,suburb=?,start_date=?,notes=?,updated_at=? WHERE id=?").bind(text$1(payload.code).toUpperCase(), text$1(payload.name), text$1(payload.service), text$1(payload.stage), Math.min(100, Math.max(0, number(payload.progress))), text$1(payload.customerName), text$1(payload.suburb), text$1(payload.startDate), text$1(payload.notes), now, id).run();
		return;
	}
	if (resource === "request") {
		await db.prepare("UPDATE job_requests SET status=?,priority=?,assigned_to=?,summary=?,updated_at=? WHERE id=?").bind(text$1(payload.status), text$1(payload.priority), text$1(payload.assignedTo), text$1(payload.summary), now, id).run();
		return;
	}
	if (resource === "schedule") await db.prepare("UPDATE schedule_events SET event_date=?,start_time=?,title=?,assignee=?,project_code=?,tone=?,notes=? WHERE id=?").bind(text$1(payload.eventDate), text$1(payload.startTime), text$1(payload.title), text$1(payload.assignee), text$1(payload.projectCode), text$1(payload.tone, "gold"), text$1(payload.notes), id).run();
}
async function deleteAdminRecord(resource, id) {
	await requirePermission(resource);
	if (resource !== "schedule") throw new Error("Admin can only remove schedule items.");
	await (await database()).prepare("DELETE FROM schedule_events WHERE id=?").bind(id).run();
}
//#endregion
...
async function loginKey$2(email, request) {
...
async function POST$6(request) {
	...
	const key = await loginKey$2(email, request);
	...
	const result = await processStaffSignIn(email, payload.password ?? "", payload.teamCode?.trim() ?? "").catch(() => ({ status: "invalid" }));
	...
	return Response.json({
		...
		"Set-Cookie": adminSessionCookie(token, secure),
		"Cache-Control": "no-store"
	} });
}
//#endregion
...
//#endregion
//#region app/api/owner/login/route.ts
var route_exports$5 = /* @__PURE__ */ __exportAll({ POST: () => POST$3 });
async function loginKey$1(email, request) {
	const bytes = new TextEncoder().encode(`${email.toLowerCase()}|${request.headers.get("cf-connecting-ip") ?? "local"}`);
...
async function POST$3(request) {
	...
	const email = p.email?.trim().toLowerCase() ?? "", password = p.password ?? "", key = await loginKey$1(email, request), attempts = await getLoginAttempt(key), now = Date.now();
	if (attempts && attempts.lockedUntil > now) return Response.json({ error: "Too many attempts. Try again in 15 minutes." }, { status: 429 });
...
//#endregion
//#region app/api/requests/route.ts
var route_exports$3 = /* @__PURE__ */ __exportAll({ POST: () => POST$1 });
var allowedServices = new Set([
	"Kitchen Renovations",
...
function text(form, name, max = 500) {
...
async function POST$1(request) {
	...
	const input = {
		...
		details: text(form, "details", 4e3)
	};
	if (!allowedServices.has(input.service) || !input.customerName || !input.phone || !input.location || !input.timeframe || !input.budget || !input.material || !input.details || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(input.email) || input.service === "Other" && input.otherService.length < 30) return Response.json({ error: "Please complete every required field with valid information." }, { status: 400 });
	const files = form.getAll("files").filter((item) => item instanceof File && item.size > 0);
	if (files.length > 5 || files.some((file) => file.size > 10 * 1024 * 1024) || files.reduce((total, file) => total + file.size, 0) > 25 * 1024 * 1024 || files.some((file) => file.type && !allowedTypes.has(file.type))) return Response.json({ error: "Upload up to 5 supported files, maximum 10 MB each and 25 MB total." }, { status: 400 });
	try {
		const result = await createPublicRequest(input, files);
		return Response.json({
			ok: true,
			code: result.code,
			redirect: `/track/${encodeURIComponent(result.code)}`
		}, {
			status: 201,
			headers: { "Cache-Control": "no-store" }
		});
...
var OwnerLogin_default = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'default' is called on server");
}, "f4e178564a02", "default");
//#endregion
...
async function OwnerPage() {
	const s = await verifyOwnerSession((await cookies()).get(ownerCookieName())?.value);
	if (!s) return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(OwnerLogin_default, {});
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(OwnerDashboard_default, {
		ownerName: "Ali Mobini",
		ownerEmail: s.email
	});
}
//#endregion
...
async function PendingStaffPage() {
		...
		children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("section", {
			...
			children: [
				...
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("p", { children: [
					"Your identity and role are active. The dedicated ",
					session.role,
					" tools will be built in the next role phase without changing your approved access."
				] }),
...
//#endregion
//#region app/api/track/[code]/route.ts
...
var routes = [
	{
		...
		ids: {
			"route": "route:/api/track/:code/messages",
			"page": null,
			"routeHandler": "route-handler:/api/track/:code/messages",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/track/:code/messages",
		patternParts: [
			"api",
			"track",
			":code",
	...
	},
	{
		...
		ids: {
			"route": "route:/api/owner/login",
			"page": null,
			"routeHandler": "route-handler:/api/owner/login",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/owner/login",
		patternParts: [
			"api",
			"owner",
			"login"
	...
	},
	{
		...
		ids: {
			...
			"page": null,
			"routeHandler": "route-handler:/api/requests",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
	...
	},
	{
		...
		ids: {
			"route": "route:/api/track/:code",
			"page": null,
			"routeHandler": "route-handler:/api/track/:code",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/track/:code",
		patternParts: [
			"api",
			"track",
			":code"
