(function(global){
  ...
  async function signUp(email,password,companyName,teamCode='',accountType=''){const redirect=authRedirect(),data=await request(`/auth/v1/signup?redirect_to=${encodeURIComponent(redirect)}`,{method:'POST',body:JSON.stringify({email,password,data:{organisation_name:companyName||'My Company',team_code:String(teamCode||'').trim().toUpperCase(),account_type:String(accountType||'')}})});if(data.access_token){saveSession(data);sessionStorage.removeItem(PRESENCE_SENT_KEY)}await loadProfile();return data}
