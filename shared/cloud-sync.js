(function(global){
  ...
  async function context(){await global.ACAuth?.ready;const profile=global.ACAuth?.profile();if(!global.ACAuth?.hasAccess?.()||!profile?.organisation_id)return null;return{profile,canSave:global.ACAuth?.canSave?.()===true,headers:{apikey:config.publishableKey,'Content-Type':'application/json',...(await global.ACAuth.headers())}}}
  ...
  async function pull(){
    ...
    try{const response=await fetch(endpoint(ctx.profile)+'&select=organisation_id,workspace,revision,updated_at',{headers:ctx.headers});if(!response.ok)throw new Error(`Cloud sync ${response.status}`);const row=(await response.json())[0],local=global.ACProjects.snapshot(),localTime=localStamp(local),remoteTime=String(row?.updated_at||'');lastRemote=remoteTime;
      ...
      if(remoteProjects.length&&(!ctx.canSave||(remoteTime&&remoteTime>localTime))){global.ACProjects.replaceSnapshot(row.workspace);setCheckpoint({remoteTime,revision:row.revision,at:new Date().toISOString()})}
      else if(ctx.canSave&&local.projects.length&&(!remoteProjects.length||localTime>remoteTime))await push(true);else status('online',ctx.canSave?`Last synced ${new Date().toLocaleTimeString('en-AU',{hour:'numeric',minute:'2-digit'})}`:'View only • cloud source');
    }catch(error){status('error',error.message)}finally{pulling=false}
  }
  async function push(fromPull=false){
    if(!config.cloudSyncEnabled||!global.ACProjects)return;const ctx=await context();if(!ctx){status('');return}if(!ctx.canSave){status('online','View only • saving requires Builder/Estimator or Owner/Admin');return}const data=global.ACProjects.snapshot(),stamp=localStamp(data)||new Date().toISOString();status('syncing');
