...
import TaskManagementPanel from "../tasks/TaskManagementPanel";
import TaskInbox from "../tasks/TaskInbox";
import OperationsControlPanel from "../operations/OperationsControlPanel";
import styles from "./admin.module.css";

type View = "overview" | "workflow" | "issues" | "followups" | "workers" | "tasks" | "projects" | "requests" | "schedule" | "messages";
...
const scheduleBlank = { eventDate: melbourneDate(), startTime: "08:00", title: "", assignee: "Site Supervisor 01", projectCode: "Business / General", tone: "gold", notes: "" };

function currentWeek() {
  const today = new Date(`${melbourneDate()}T12:00:00+10:00`);
  ...
  return Array.from({ length: 7 }, (_, index) => {
    ...
    return {
      key: new Intl.DateTimeFormat("en-CA", { timeZone: "Australia/Melbourne", year: "numeric", month: "2-digit", day: "2-digit" }).format(date),
      day: new Intl.DateTimeFormat("en-AU", { timeZone: "Australia/Melbourne", weekday: "short" }).format(date),
      date: new Intl.DateTimeFormat("en-AU", { timeZone: "Australia/Melbourne", day: "numeric" }).format(date),
    };
  });
}
...
function todayLabel() {
  return new Intl.DateTimeFormat("en-AU", { weekday: "long", day: "numeric", month: "long", year: "numeric", timeZone: "Australia/Melbourne" }).format(new Date());
}
...
export default function AdminDashboard({ viewerName, viewerEmail, previewAsOwner }: { viewerName: string; viewerEmail: string; previewAsOwner: boolean }) {
  ...
  const headers: Record<View, [string, string, string]> = {
    ...
    issues: ["Immediate site escalation", "Site delays & problems", "Review Site Supervisor reports, record the action and reschedule the next site follow-up."],
    followups: ["Daily task continuity", "Upcoming follow-ups", "Record tomorrow’s priorities at clock-out and keep every open follow-up visible."],
    workers: ["Controlled trade access", "Worker assignments", "Assign approved Workers, issue tasks, choose file access and review End-of-Day reports."],
    tasks: ["Operational direction", "Team task centre", "Assign tasks to Site Supervisors, Electricians, Plumbers and other approved field Workers."],
  ...
  return <main className={styles.adminShell}>
    ...
    <section className={styles.mainPanel}>
      <header className={styles.topbar}><div className={styles.mobileBrand}><BrandLogo kind="tradie" tone="dark" className={styles.mobileLogo} /></div><div><span>{todayLabel()}</span><strong>Alert Tradie Pro · Operation Hub</strong></div><div className={styles.profileChip}><span>AD</span><div><strong>{previewAsOwner ? "Admin preview" : viewerName}</strong><small>{viewerEmail}</small></div></div></header>
      <div className={styles.content}>
        ...
        {loading ? <div className={styles.loading}><i />Opening live operations…</div> : <>
          ...
          {view === "issues" && <OperationsControlPanel role="admin" mode="issues" preview={viewerEmail.includes(".local")} />}
          {view === "followups" && <OperationsControlPanel role="admin" mode="followups" preview={viewerEmail.includes(".local")} />}
          {view === "workers" && <WorkerManagementPanel role="admin" />}
